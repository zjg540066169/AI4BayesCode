...
def _offline_emit(p: dict, output_path: str, verbose: bool) -> dict:
    ...
    rf.write_text(
        ...
        "  (B) Online: set a key once with AI4BayesCode.set_key(\"sk-ant-...\", \"anthropic\")\n"
...
def set_key(key: str, provider: str = "anthropic", check: bool = True) -> str:
    ...
    Args:
        key: Non-empty API-key string (e.g. ``"sk-ant-..."``). Placeholder forms
            containing ``...``, ``<>``, or "YOUR KEY" are REJECTED on purpose,
            so a copied-but-unedited example fails loudly instead of being
            stored as a key.
