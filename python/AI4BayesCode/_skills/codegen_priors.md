...
Sec.0 and `codegen.md` Sec.0): **in Claude Code, use the `AskUserQuestion` tool -- never
...
fallback from `start.md` Sec.0.** Either way, NEVER skip the ask or silently use a
default. This applies to every "ask the user" moment below, including: prior on
variance / scale (Sec.2a), parameterization ambiguity (Sec."Parameterization
ambiguity"), missing hyperparameters on Gamma / Beta / Laplace / etc.,
conjugate-vs-NUTS block choice, and the discrete-variable decision tree (Sec.3a).
...
`AskUserQuestion`. The same rule is in `codegen.md` Sec.0.
...
- Sec.2a: scale-prior choice (Jeffreys / HalfNormal / Inverse-Gamma /
  literature-informed).
...
`+log(sigma)` Jacobian, yielding a clean likelihood on eta-space.
...
`(2pi sigma^2)^{-N/2}`. That -N log(sigma) is part of the LIKELIHOOD,
...
(the Jacobian). Example for Gaussian regression:
    ...
    natural-scale lp(sigma) = -(N+1) log(sigma) - SSE / (2 sigma^2)
                              ^^^^^^^^^^^^^^^^
    ...
    unconstrained lp(eta)      = -N log(sigma) - SSE / (2 sigma^2)
                              = -N*eta - SSE / (2 exp(2eta))
...
- `InverseGamma(epsilon, epsilon)` with small epsilon (Gelman 2006 Bayesian Analysis
  1(3):515-533 critique: improper at epsilon -> 0; posterior is highly
  sensitive to the choice of small epsilon; not actually noninformative).
...
(sum_active beta^2 = 0); group-level variance when a group has 0
...
2. **Detect k=0 inside the natural-scale log-density** and return
   ...
   spike-and-slab sigma^2tau^2 parametrization, tau=1 is "signal scale =
   noise scale"). This is a transient state; as soon as any gamma_j
...
fallback. The Ishwaran & Rao 2005 JASA sigma^2tau^2 parametrization (tau
...
`rjmcmc_block`, per Sec.3a), sparse GMRF / CAR / ICAR, GP latent, tree
...
1. **`joint_nuts_block`** -- **DEFAULT for continuous parameters.**
   ...
   `constraints::<kind>::wrap` -- system_design.md Sec.10.1).
   ...
   **Do NOT force a decomposition to fit existing blocks.** Block-
   ...
   VAR / spectral coupling) mix ~10x slower per ESS when split, and
...
3. **`rjmcmc_block`** -- trans-dimensional. Class 4; only applicable
   ...
   Sec.11.2(a) of `system_design.md`.
4. **`bart_block` / `genbart_block`** -- tree-based nonparametric
   ...
   - `genbart_block`: Linero 2022 generalized-BART via **RJMCMC** with
     ...
     unknown-variance heteroscedastic / user-supplied). WARNING **Do NOT use
...
6. **`pg_logistic_block`** -- Bayesian logistic regression via
   ...
   for p < 1000.
   WARNING **LINEAR LOGISTIC ONLY.** Hard-scope limitation: the predictor
...
7. **Gaussian Process blocks** -- architecture chosen by the
   ...
   |---|---|---|
   | Gaussian | YES (`p(y\|theta) = N(y \| 0, K + sigma^2 I)`) | **Marginal**: sample only hyperparameters |
   ...
   targets `y ~ N(0, K + sigma^2 I)`. Analytic gradient via Rasmussen &
   Williams Sec.5.5 Eq. (5.9). f is recovered at predict time via the
...
8. **`gmrf_precision_block`** -- Gaussian Markov Random Field latent
   ...
   prior takes the form `pi(x) prop.to exp{-1/2 x^T Q x + b^T x}` with Q
...
9. **`ising_cluster_block`** -- Discrete Markov Random Field on a
   ...
   cluster moves. Use when latent is x  in  {0..Q-1}^n with prior
   pi(x) prop.to exp{beta Sigma_{i~j} I[x_i = x_j]}. Reference template:
...
10. **`gmrf_whitened_ess_block`** -- Gaussian Markov Random Field latent
    ...
    `x' = x cos theta + nu sin theta` against the user-supplied
    log-likelihood. Optional sum-to-zero (preserved invariantly by the
    ESS rotation). Use whenever the latent's prior is
    `pi(x) prop.to exp{-1/2 x^T Q x}` (Q sparse PSD) AND the observation
...
- **Exception 3 -- Scalar / vector textbook conjugate with NUTS-wasteful
  efficiency profile.** Example: pi in spike-and-slab has only a
  length-p sufficient statistic and posterior Beta(a + Sigmagamma,
  b + p - Sigmagamma) -- NUTS warmup on 1-D tight-posterior scalar is
  wasteful. Another example: heavy-tail IG conditional triggering
  NUTS adaptation pathology (but prefer the Sec.2a pattern --
...
- `inv_gamma_gibbs_block` (DISCOURAGED as default -- see Sec.2a):
...
The codegen.md Sec.3 VI engine trigger lets the user opt into pure VI
...
parameters lambda; if the prior contributes a non-finite term at the
typical eta scale, the optimizer diverges. MCMC tolerates a much
...
- Jeffreys `p(sigma) prop.to 1/sigma` on a VI'd scale: improper, but conditionally
  ...
  when the natural-scale gradient explodes near sigma -> 0. Use the
  Sec.2a inline k=0 half-Normal(0, 1) fallback pattern, OR switch the
  VI'd sigma to half-Normal(0, A) with A ~= 2.5 x data-scale. For pure
  MCMC the Sec.2a defaults stand unchanged.
- Flat / uniform on R for a location parameter: never well-defined
  ...
  informative N(0, 5^2) or similar.
- "Diffuse" priors with very large scale (N(0, 1e6^2), Gamma(1e-3,
  ...
  scale, so initial sigma values are far from a useful neighborhood.
  Use weakly informative defaults (N(0, 5^2) or N(0, 10^2) on
  regression coefs; half-Normal(0, 2.5 x data-scale) on sigma).
...
- For hierarchical models on MCMC, non-centering is sometimes a
  ...
  posterior fails badly (the prior-scale coupling between theta_j and
  tau creates posterior dependence that mean-field cannot capture).
  Default to non-centered for any VI'd hierarchical parameter.
- For BNN weights: keep them in the natural N(0, sigma^2) form (no
  reparameterization needed); the VI block sees them as
  `q(eta_jk) = N(eta_jk; mu_jk, sigma_var,jk^2)` directly. The prior sigma^2
...
eta is exactly the same as NUTS's unconstrained-space draw, with
the same log|J|. No new prior-elicitation work is needed for the
transform.

**(4) Hyperpriors on hyperparameters (sigma2_alpha, sigma2_beta
in BNN, tau in Lasso, etc.)** typically belong on MCMC even in
...
**(5) Spike-and-slab (Sec.3a Class 2a Gaussian-mixture spike or Sec.3a
...
(the q(gamma_j) marginal collapses to a deterministic 0 or 1 under
...
See `system_design.md Sec.18.5` (variational family) and Sec.18.9
...
|---|---|---|
| `Normal(0, 1)`        | sigma=1 <=> sigma^2=1 (since 1^2=1)              | **No** |
| `Normal(0, 4)`        | sigma=4 vs sigma^2=4 (i.e. sigma=2) -- different   | **Yes** |
| `Gamma(1, 1)`         | rate=1 <=> scale=1                     | **No** |
| `Gamma(2, 1)`         | rate=1 <=> scale=1, shape irrelevant   | **No** |
| `Gamma(2, 3)`         | rate=3 vs scale=3 -> 9x different     | **Yes** |
| `HalfNormal(1)`       | sigma=1 <=> sigma^2=1                           | **No** |
...
a BART mean component), **override the generic Jeffreys `p(sigma) prop.to 1/sigma`
...
`codegen.md Sec.2`), use WebSearch and/or PubMed tools to find
...
## 3a. Discrete-variable decision tree (RUN THIS BEFORE Workflow Sec.4)
...
architectural reasons live in `skills/system_design.md` Sec.11 --
...
+-- z's structure -------------------------------------------------
    ...
    CLASS 1: per-observation K-way, {z_i} conditionally independent
             given continuous theta.  Examples: Gaussian mixture,
             latent-class, ZIP / ZINB, mixture regression, LCA.
    -> USE `categorical_gibbs_block` (or `binary_gibbs_block` if K=2).
      pi can go in `dirichlet_gibbs_block` (conjugate) or nuts_block
      on simplex. Component params (mu, sigma, beta) go in nuts_block.
    ...
    -> USE `probit_aug_block` for the latent z (Exception 3 -- closed-
      ...
      Gaussian-likelihood block for the mean structure:
        - linear mean (beta):    nuts_block on beta with N(0, prior_sd^2) prior;
    ...
    CLASS 2a: continuous spike-and-slab. Both components have positive
              Gaussian variance (tight spike tau_0 > 0 plus wider slab tau_1).
    -> USE `binary_gibbs_block` for gamma_j + nuts_block for beta_j.
      Gibbs is valid (both Gaussians have positive density everywhere).
      When tau_0/tau_1 < 0.01 (VERY tight spike), the Gibbs chain mixes
      slowly; in that specific case you may marginalize gamma inside beta's
      NUTS lambda using a log-sum-exp on the 2-component prior. Only
      do this if the user explicitly asks or tau_0 is pathologically
      tight. Default: plain Gibbs.

    CLASS 2b: Dirac spike-and-slab (beta_j is EXACTLY 0 when gamma_j = 0;
    ...
    -> *** SUPPORTED via `rjmcmc_block` v0. ***
      ...
      Structure (see Sec.2a, Sec.2b):
        - pi:    beta_gibbs_block (Exception 3 -- scalar textbook
                 conjugate; has library parity test Check #15)
        - sigma: nuts_block with Jeffreys p(sigma) prop.to 1/sigma
        - tau:   nuts_block with Jeffreys p(tau) prop.to 1/tau, with
                 k=0 fallback to half-Normal(0,1) pin (see Sec.2a's
      ...
      Jeffreys priors.
      Do NOT attempt:
        - `binary_gibbs_block` + fixing beta=0 when gamma=0: state space
          $\{(0,0)\} \cup \{1\}\times\mathbb{R}$ is not a fixed-dim
          manifold; Gibbs is NOT irreducible.
        - marginalizing gamma + NUTS on beta: marginal prior is a mixed
      ...
      See system_design.md Sec.10.2 for the Jacobian story (identity-
