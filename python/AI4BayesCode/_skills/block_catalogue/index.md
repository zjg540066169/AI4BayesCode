...
| **discrete MRF / Ising / Potts on a graph** (`pi(x) prop.to exp{beta Sigma_{i~j} I[x_i = x_j]}`, x  in  {0..Q-1}^n) | **`ising_cluster_block`** (Swendsen-Wang 1987 cluster moves) | **(none -- bond augmentation + per-cluster recolor)** |
| **sparse-precision Gaussian MRF** (`pi(x) prop.to exp{-1/2 x^T Q x + b^T x}`, Q sparse PSD, optional sum-to-zero) | **`gmrf_precision_block`** (Rue 2001 sparse Cholesky + AMD) | **(none -- direct conjugate draw via Eigen SimplicialLLT)** |
| **Gaussian-data GMRF with UNKNOWN smoothing precision** (`y ~ N(x, sigma^2I)`, `x ~ N(0,(kappaR)^-1)`, `kappa ~ Gamma`; slow (x,kappa) Gibbs mixing) | **`gmrf_gaussian_joint_block`** (Knorr-Held & Rue 2002 collapsed joint (x,kappa) update) | **(none -- kappa from its marginal via sparse-Cholesky log-det, then x\|kappa,y)** |
| **sparse-precision GMRF + non-Gaussian likelihood** (`pi(x) prop.to exp{-1/2 x^T Q x}` * `log_lik(x)`; Poisson / Bernoulli / Student-t / NB / log-Gaussian Cox observation) | **`gmrf_whitened_ess_block`** (Murray 2010 ESS on implicit GMRF prior; Rue 2001 backsolve for prior draws) | **(none -- Eigen SimplicialLLT + permuted backsolve + likelihood-free ESS shrink)** |
| **Bayesian-network structure learning** (discrete data, n <= 64, BDeu score; output = total order < + Bayesian-model-averaged DAG over Pa(i) subset Pred(i, <)) | **`order_mcmc_block`** (Friedman-Koller 2003 order MCMC + Heckerman-Geiger-Chickering 1995 BDeu + FK Sec.4.2 three-tier cache) | **(none -- combinatorial MH on permutations)** |
...
| **mixture-component atoms across K_trunc clusters, given explicit allocations z -- ANY per-component prior, ANY component density. THE DEFAULT: start here and leave only for an EXACT conjugate match below.** | **`cluster_atom_block`** | **Ishwaran & James (2001) step (a) per component, univariate slice; empty components fall back to the prior automatically. Do NOT fall back to one `joint_nuts_block` over all K components -- one frozen step size cannot track a target that flips prior-wide / data-narrow every sweep.** |
| **the same, but the per-component prior is EXACTLY conjugate diagonal Normal-Gamma** (a Normal location with an independent half-Normal / half-Cauchy / log-Normal scale is NOT Normal-Gamma -- that takes the default above) | **`normal_gamma_cluster_gibbs_block`** | **(none -- per-cluster, per-dim conjugate)** |
...
| **interval-censored survival data augmentation (each subject's event known only to lie in `(L_i, U_i]`; right-censored `U_i = +Inf` supported) under a piecewise-exponential baseline hazard; imputes latent `T_i` so the sibling PEH-Gibbs block sees complete data** | **`interval_censored_survival_augmentation_block`** (Tanner-Wong 1987 augmentation; Sinha-Chen-Ghosh 1999; Lin-Wang 2010) | **(none -- exact inverse-CDF sample from truncated PEH conditional)** |
| **shared-frailty proportional-hazards survival (per-group `w_g ~ Gamma(theta, theta)` frailty multiplies each subject's hazard; theta fixed or sampled by a sibling block); composes with `piecewise_exponential_gibbs_block` (baseline) + a regression sibling for the log-relative hazard** | **`frailty_gamma_gibbs_block`** (Clayton 1991; Ibrahim-Chen-Sinha 2001 Sec.4.3; Vaupel-Manton-Stallard 1979) | **(none -- exact Gamma-Gamma conjugate draw of `w[G]`)** |
...
- Spike-and-slab mixing proportion: `gamma_j ~ Bernoulli(pi)`, `pi ~ Beta(a, b)`
  ...
  (Sigma gamma). NUTS warmup on 1-D tight-posterior scalar is
