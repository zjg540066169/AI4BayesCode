...
|---|---|---|
| **NUTS-gradient** (default for MULTI-dim / coupled fixed-dim continuous) | smooth log-density; hand-written natural-scale logp + grad | `set_current(const arma::vec&)` unconstrained; gradient Check #12 (semantic AD-verify via a throwaway compile at generation, see `jacobian.md` Sec.10.1) |
...
| **Slice / ESS** | any SCALAR sampled in its own block -- the default there (`univariate_slice_sampling_block`, `codegen_priors Sec.2b.1`); also 1-D non-diff/black-box, or a GMRF **prior** under a **NON-Gaussian** likelihood (Murray 2010 ESS). NOTE: if the likelihood is Gaussian-conjugate the site conditionals ARE closed-form -> use the **Gibbs** row above, not ESS | metric-free; no dense-metric machinery |
...
`step(std::mt19937_64&)`, `current()`, `set_current(const arma::vec&)`, `dim()`.
