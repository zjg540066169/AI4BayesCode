...
|---|---|---|
| **NUTS-gradient** (default for fixed-dim continuous) | smooth log-density; hand-written natural-scale logp + grad | `set_current(const arma::vec&)` unconstrained; gradient Check #12 (semantic AD-verify via a throwaway compile at generation, see `jacobian.md` Sec.10.1) |
