<!-- system_design MODULE -- Jacobian discipline (extracted 2026-06-21 from system_design.md Sec.10).
...
### Sec.10.1 NUTS-family -- constraints::<kind>::wrap
...
of the same math, and compares 5-20 random-theta points. Max diff must
...
See `skills/validator.md` Sec.12 for the full template.
...
### Sec.10.2 RJMCMC-family -- three-tier Jacobian story
...
Birth proposes a new coordinate `beta_j_new ~ q(beta | context)`
directly; the proposed beta_j IS the auxiliary variable u. The
bijection `(x, u) -> (x', u')` is the identity, `|det J| = 1` by
construction, and the accept ratio contains no Jacobian term.

**The user supplies:**

- `log_joint(gamma, beta, ctx)` -- joint log density on (gamma, beta) given
  everything else (NATURAL scale). Standard Check #5 applies
  (no `+ log(...)` Jacobian correction).
- `propose_sample(rng, j, ctx)` -- draws a new beta_j.
- `propose_logq(beta_new, j, ctx)` -- log q evaluated at beta_new.
- `continuous_update(rng, j, ctx)` (optional) -- direct update of
  beta_j when gamma_j = 1; typically a closed-form Gibbs draw from the
  conditional posterior. Critical for good mixing; without it beta_j
  stays at its birth-time value for as long as gamma_j = 1.
...
`beta = sigma x u` for u ~ N(0,1)"), `rjmcmc_block_config` accepts an
...
= diagonal_affine):
- `identity_transform_1d`       (beta = u, |J| = 1)
- `diagonal_linear_transform_1d(scale)` (beta = scale x u, |J| = |scale|)
- `diagonal_affine_transform_1d(scale, offset)` (beta = scale x u + offset)
...
u**, not on beta_new. The library transforms u -> beta and computes the
...
- `double` for the sampling step (`beta = T(u)`),
...
**Updated Check #14 (SHIPPED, see validator.md Sec.14):** three
...
2. **Jacobian non-singularity**: `|dbeta/du| > 1e-12` -- catches
