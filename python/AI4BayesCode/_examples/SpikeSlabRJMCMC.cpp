...
//       function(seed) new(SpikeSlabRJMCMC, X, y, rng_seed = seed, keep_history = TRUE),
...
//   m <- new(SpikeSlabRJMCMC, X, y, rng_seed = 7L, keep_history = TRUE)  # Beta(1,1) on pi
...
public:
    SpikeSlabRJMCMC(const arma::mat& X,
                    const arma::vec& y,
                    double           a_pi_prior = 1.0,
                    double           b_pi_prior = 1.0,
                    int              rng_seed = 1,
