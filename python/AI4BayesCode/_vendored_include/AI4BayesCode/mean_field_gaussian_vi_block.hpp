...
public:
    ...
    void step(std::mt19937_64& rng) override {
        if (converged_) {
            ...
            // joint-draw consumer to the pre-convergence prefix. Hold the
            // last values in place, the same semantics record_held_history()
            // uses for a frozen block. current_sample still draws fresh
            // samples from the frozen q.
            if (keep_history()) {
