...
Forest::Forest(const Hypers& h, const Opts& o) : hypers(h), opts(o) {
  trees.resize(hypers.num_tree);
  saved_forests.resize(0);
  for (int i = 0; i < hypers.num_tree; i++) { trees[i] = new Node(); trees[i]->Root(hypers); }
  num_gibbs = 0;
  tree_counts = zeros<umat>(hypers.s.size(), hypers.num_tree);
}
...
void Hypers::finalize_from_group() {
  // pure-C++ equivalent of the derived setup in Hypers(Rcpp::List).
  sigma_mu_hat = sigma_mu;
  num_groups = group.max() + 1;
  s = 1.0 / num_groups * arma::ones<arma::vec>(num_groups);
  logs = log(s);
  group_to_vars.resize(s.size());
  for (int i = 0; i < (int)s.size(); i++) group_to_vars[i].resize(0);
  int P = group.size();
  for (int pp = 0; pp < P; pp++) { int idx = group(pp); group_to_vars[idx].push_back(pp); }
  int GRID_SIZE = 1000;
  rho_propose = arma::zeros<arma::vec>(GRID_SIZE - 1);
  for (int i = 0; i < GRID_SIZE - 1; i++) rho_propose(i) = (double)(i+1) / (double)(GRID_SIZE);
}
