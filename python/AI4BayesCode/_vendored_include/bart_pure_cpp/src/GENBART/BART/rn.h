...
// Pure-C++ (R-free) build: the standalone R-API shim must be included at
// GLOBAL scope (before `namespace genbart`) so its `namespace R {...}` and
// bare unif_rand/norm_rand stay global and genbart::arn's R::norm_rand()
// resolves to ::R. No-op under the Rcpp build. See src/r_compat.h.
#ifdef NoRcpp
#include "../../r_compat.h"
#endif
...
#ifndef NoRcpp
  Rcpp::RNGScope RNGstate;   // R-build only; pure-C++ RNG (r_compat.h) owns state
#endif
};
