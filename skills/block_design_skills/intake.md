...
> license gate (`00_flow.md §0.5`) fires BEFORE this phase. If you reached `intake.md`
...
`start.md §0` ("Concrete mechanism per environment"). Mark the standing
...
> likelihood, the cluster parameters, or whether the concentration α is fixed or
...
format per `start.md §3b`; never inline `$...$` — and any roles / I-O TABLE you show has
plain-Unicode cells like `X`, `β`, `ℝᵖ`, `n×p`, `>0`, NEVER `$...$`, since a `$$` display
block cannot live inside a table cell):

1. **Likelihood / observation model FIRST — present it, then CONFIRM it.** Show
   `p(y | θ, ...)` (the full observation model) as a `$$ ... $$` **display block**
   ...
   `start.md §0`: `AskUserQuestion` in Claude Code, the markdown labeled-option fallback
...
2. **THEN elicit the priors + supports** (once the likelihood is confirmed):
   - **Every prior** `p(θ)`, `p(hyper)`,... — each with its parameterization stated (e.g.
     Gamma **shape-rate vs shape-scale** — the Check #1 bug class, `validator.md §1`); no
     "standard prior" hand-waving.
   - **Each parameter's SUPPORT** — real / positive / simplex / interval / ordered /
     Cholesky-corr / unit-vector / binary / categorical (`system_design §11` geometry rule 1;
...
- **Refreshable context (read each sweep from `shared_data` via `ctx.at(key)`)** —
 ...
 counts, an imputed design matrix (`system_design §3`). The block READS, never
 owns, these.
...
in the math (`system_design §3` lists hyperparameters as shared_data entries; §4's generative DAG
...
- usually fixed for a run → a **config field with a sensible default** that is ALSO **adjustable
 ...
 stored value + any shared_data mirror; `dataflow §7`), so it is tunable post-construction WITHOUT
...
- could be **inferred** by a sibling block / changed online every sweep → a **refreshable
 ...
 external input" / full-Bayes means here, and it auto-gives the §4 context edge);
- the prior/likelihood **functional FORM** itself → a **callback** (so the user plugs N(0,τ²) /
...
> full models, with the likelihood, cluster parameters, and α living in sibling
...
(`class <Block>`, `<Block>.hpp`, `blocks_local/<Block>/`; the manifest `Block:` field, schema in `skill.md §3`).
...
4. **Unique vs core + existing local** — check at creation against the
 core `block_catalogue.md` entries and every existing
...
- The **authoritative reserved-name list = the shipped block headers**:
 ...
 reserved core name). The routing table in `AI4BayesCode/skills/block_catalogue.md`
...
Two DISTINCT fields, two readers (`skill.md §3`), elicited in THIS ORDER — do not conflate
...
`yᵢ ~ Student-t(ν, xᵢᵀβ, σ²)`). (2) The prior on each parameter. (3) One plain sentence on what
it is good for. (4) The **sampling strategy matters too** — add ONE plain, high-level sentence
on how it is sampled ("β and σ are sampled jointly with the No-U-Turn sampler"), but do NOT
lead with it and do NOT dwell on deep implementation ("single NUTS kernel over (β, log σ)",
...
yᵢ ~ Student-t(ν, xᵢᵀβ, σ²), for robust inference when residuals contain gross outliers. The
coefficients β take a weakly-informative Gaussian prior and the scale σ a half-Normal prior;
the degrees of freedom ν is a fixed, tunable setting. β and σ are sampled jointly with the
...
`start.md §0`), then a SHORT confirm (`Use it (default) / Refine — tell me the change /
...
`start.md §0`), then a SHORT confirm (`Use it (default) / Refine / Other`). **A vague OR bloated
...
- **Composable** (existing blocks + NC would reach the same posterior): say so plainly, and the
 ...
 budget, `joint_nuts_block`+NC vs the proposed block, ESS/sec + R̂ side by side) — offered to
...
/ `libgp_kernels`, already on `-I` — `system_design §2` Tier-C), this step
...
(authoritative statement: `system_design §14` Step 1 = `lifecycle.md`; do not restate, apply it):
...
(2) fire a **structured confirm gate** — `AskUserQuestion` in Claude Code (the §0 markdown fallback
...
- Geometry / parameter support classes → `system_design §11` (`geometry.md`).
- Three-tier architecture + FIXED vendored set + `shared_data`/`block_context`
 → `system_design §2`/`§3` (`interface.md` / `dataflow.md`).
- License-compatibility rule (authoritative) → `system_design §14` Step 1
 (`lifecycle.md`).
- Constraint/transform kinds (used in DESIGN's constraint mapping) →
 `constraints.md`.
- Parameterization (shape-rate vs scale) bug class → `validator.md §1`.
- Manifest field definitions (the full 13-field schema) → `skill.md §3`
...
- The two-stage auto-selection + compact index that `SelectWhen` feeds →
 `contrib.md §4` (Discovery + auto-selection).
