...
**If any prerequisite is missing -> route to `fail/` immediately (see Sec.3).**

The reference type determines the cross-impl harness (Sec.6 below):
...
single-chain split-rhat pattern (Sec.6 below) -- `posterior::rhat()` on a
...
align_REF_draws <- function(ref_run) {
    fit <- ref_run$fit
    # BART::wbart returns an n_keep x N matrix in $yhat.train and a
    # length-n_keep vector in $sigma.
    list(
        f_bart = as.matrix(fit$yhat.train),    # (n_keep x N), per-obs f
...
|---|---|---|
| Continuous BART, Gaussian outcome | `BART::wbart(x.train, y.train, ntree, nskip, ndpost)` | Returns `$yhat.train` (n_keep x N), `$sigma` (n_keep). Default `ntree = 200`; use the value from `<id>.r`. |
...
| Soft BART, Gaussian | `SoftBart::softbart(X, Y, X_test, hypers, opts)` | Per-package `Hypers()` includes `normalize_bart(Y)` + `cv.glmnet`-derived `sigma_hat` (see `codegen_r_runner.md Sec.9a`); the sim1's REF call MUST use the same `Hypers()` to keep the prior match |
...
| Joint NUTS over a 3-vec on log-scale (e.g., several POSITIVE PK params) | `k_a, K_m, V_m` (split + transformed) | `theta_log` (n_iter x 3, log-scale; need `exp()`) |
| Joint NUTS over a simplex with auxiliary slack (e.g., GARCH-style `(alpha1, beta1, slack)`) | `alpha1, beta1` (split) | `ab_simplex` (n_iter x 3) -- first two columns are `alpha1, beta1` |
...
`sigma_hat` per-call (see `codegen_r_runner.md Sec.9a`). If sim1's
