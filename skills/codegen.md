...
## ⚠️ Token discipline — DO NOT preload sub-skills
...
- `codegen_priors.md` — load when entering **§2 (Prior specification)**,
...
- `codegen_cpp.md` — load only when entering **§5+ (C++ emission)**.
  Skip until then.
...
runtime (see `start.md` §0 "Concrete mechanism per environment"):
...
- **In any environment WITHOUT a structured-question tool** (Codex Default mode,
  ...
  fallback from `start.md` §0; the user replies with the option letter. NEVER
...
## 0b. Math rendering — display blocks + Unicode, NEVER inline `$...$`

This chat client renders `$$ … $$` **display blocks** but does NOT render **inline `$…$`** —
inline shows the raw source (the #1 cosmetic bug in this flow: `$\beta$`, `$y_i \sim
N(\ldots)$`, `$p(\sigma^2)\propto 1/\sigma^2$` all print as raw text). So EVERYWHERE in the
codegen conversation — the "read of your model", the §1 model-structure confirm, the prior
phase, the §3 confirmation — apply:
...
- **Any equation / expression** (a likelihood, a prior, a gradient, a multi-line model) → a
  ...
  (the §3(a) form). This renders. Do NOT inline it.
- **A lone symbol mid-sentence** (β, σ, ν, σ², ℝᵖ, xᵢᵀβ, rᵢ) → plain **Unicode**, never `$…$`.
- **Table cells** → plain Unicode ONLY — a `$$` block cannot live in a cell and inline `$…$`
  shows raw: `X y β σ ν σ² ℝᵖ n×p >0 xᵢᵀβ`.
- **NEVER** write inline `$…$` for anything you want rendered. A bulleted "Likelihood: `$y_i
  \sim N(x_i^\top\beta,\sigma^2)$`" is WRONG — lift it into a `$$` display block; `$\beta$`
  mid-sentence is just `β`; `$p(\sigma^2)\propto 1/\sigma^2$` is a `$$` block (or Unicode
  `p(σ²) ∝ 1/σ²`).
...
Per **§0 ABSOLUTE RULE**, ask these via `AskUserQuestion` before
...
marked `(prior: to be chosen)` — as ONE `$$` **display block** (§3(a) form; never inline `$…$`,
per §0b), then **fire the structured confirm gate** (§0 mechanism — `AskUserQuestion` in Claude
...
6. **Sampler engine.** Engine-level choice between MCMC and Variational
   ...
   >   this; codegen surfaces a recommendation with explanation at §X
   ...
   > - **(c) VI** — codegen will follow up at §X (VI sub-flow) on
   ...
   **Conditional follow-ups based on this answer:**
   - **(a)** → question #7 below (sampler/block preference) fires
     normally. §X VI sub-flow runs after model confirmation and may
     still recommend VI based on the spec.
   - **(b)** → question #7 fires normally. §X VI sub-flow is skipped
     entirely.
   - **(c)** → question #7 is SKIPPED (MCMC block prefs are
     irrelevant for pure VI; hybrid follow-up at §X handles which
...
7. **Suggested sampling algorithm or preferred blocks** (optional).
   **CONDITIONAL: only fire this question if the user picked (a) or (b)
   ...
   - **(a) Skip** — let the skill pick blocks via its standard
     decision rules (`codegen_priors.md §2b` block-selection priority +
     §3a discrete-variable decision tree). This is the default for
   ...
   - **(b) I have a suggestion** — accept a free-form text answer
     ...
     "no hand-written Gibbs", `system_design.md` §11.2 "what NUTS
...
### Step 0 — prior completeness routing (model structure already confirmed at the §1 Step-0 gate)

The model structure + any priors the user already gave were rendered and CONFIRMED at the
**Step 0 model-structure confirm** at the top of §1 (before any upfront question). So §2 does
...
- **Spec already COMPLETE** (every prior specified — nothing left to elicit): skip the entire
  Decision flow below and jump straight to the §3 model-confirmation gate (full LaTeX + DAG +
  summary table, signed off in one place). Do NOT run the §2 elicitation and do NOT make the
  user confirm twice.
- **Anything missing:** run the Decision flow below to elicit each missing prior (the ones
  marked `(prior: to be chosen)` at the §1 gate). No need to re-confirm the structure — that
  is already done. The full spec WITH all priors is then signed off at the §3 gate.

(Two-gate pattern: gate 1 = model structure, at §1 *before* the engine question; gate 2 =
full spec with priors, at §3.)
...
| **Scale (sigma, tau) — DEFAULT** | **Jeffreys: `p(sigma) ∝ 1/sigma`** | **none (scale-invariant)** |
| Scale — k=0 transient guard | `half-Normal(0, 1)` pin (inline fallback) | (dimensionless reference) |
| Scale — genuinely sparse (k stays < 5) | `half-Normal(0, A)` | A ≈ 2.5 × data-scale |
| Precision (1/sigma^2) | DO NOT default to `Gamma(ε, ε)` — use Jeffreys on sigma | — |
...
have domain knowledge. See `codegen_priors.md §2a` for the variance-
prior rationale.

**Before writing any code, load `codegen_priors.md`** if:
- The model has a scale/variance parameter (§2a).
- You need to pick a block type (§2b).
- Gibbs discipline matters (§2c-e).
- There is any discrete latent variable (§3a).
...
and the §6b autodiff-verify file). A future codegen agent that reads
...
`system_design.md` §4). Practical rules:
...
- Sibling priors that are independent in the generative story have
  **NO edge between them**, even if their Gibbs full conditionals
  ...
  Laplace with `β_j | γ_j=1, τ²_j, σ ~ N(0, σ²τ²_j)` and
  `τ²_j | b ~ Exp(1/(2b²))`, the predict DAG has
  `π → γ`, `b → τ²`, and `γ, τ², σ → β`, plus `β, σ, X → y`.
  σ and τ² are **siblings** (both parents of β), with **no σ↔τ²
  edge**, even though the σ-block's Gibbs conditional reads τ² and
  vice versa via the slab term σ²τ².
- Deterministic derived quantities (e.g. `linear_predictor = X β`,
...
confirmed at the **Step 0 model-structure confirm** (top of §1, before any upfront question), so
don't re-derive it here — this gate confirms the COMPLETE spec now that priors are filled in
(with the DAG + summary table). If priors were already complete (no §2 elicitation ran), this is
simply the next gate after Step 0; if priors were just elicited in §2, focus the user on the
priors + wiring
added since the §1 confirm. After presenting all three, ask the user:
...
If §1 #6 picked **(b) MCMC**, this sub-flow does NOT run.

**Always print the disclosure caveats first** (Bishop §10.1.2 /
...
**Layer-3 diagnostic implications**: VI blocks use joint PSIS-k̂
(< 0.7) instead of R-hat (Yao 2018, Dhaka 2021). Hybrid samplers
report BOTH — R-hat on MCMC children, k̂ on VI children, plus an
...
>   uniform diagnostic (joint PSIS-k̂). Use if you want a clean
...
one-line hint of why VI may help (e.g. "alpha (M=784 × J=10 =
...
- regression coefficients `β` and the noise scale `σ²` (β's marginal
  variance inflates with σ² uncertainty);
...
`(β, log σ², …)` when the merged dim ≤ 50. This is the VI analog of
...
> separate VI blocks (`q(β)·q(σ²)` coupled by composite q-sample passing)
...
> PSIS-k̂ is a FALSE reassurance: it scores fit to each block's
> CONDITIONAL (given a sibling q-sample), NOT the joint marginal, so a
> too-narrow q can show a low k̂ while its variance is wrong. A single
> joint block removes the coupling noise AND gives ONE well-defined
> joint k̂ (the only honest VI diagnostic for coupled params).

**Merge-dim fallback**: merged coupled dim > 50 → single
`mean_field_gaussian_vi_block` over the concatenated vector (still ONE
block, ONE joint k̂ — strictly better than splitting) and WARN that
...
| Discrete latent (`z_i ∈ {0..K-1}`), no known coupling structure | any | `mean_field_categorical_vi_block` |
...
- **BART / genBART — sparse variable selection (DART, Linero 2018).**
  ...
  - **Yes, enable DART** → for `bart_block`, expose `dart` and `aug`
    ...
    `cfg.hypers.dart_const_theta = false` to let θ adapt). The DART
...
- **Spike-and-slab — Dirac vs normal-approximate spike (REQUIRED).**
  ...
  **`AskUserQuestion` MUST fire** with these three choices before
  any code is written:
  - **(a) Dirac spike** — `β_j` is exactly 0 when `γ_j = 0` (point
    ...
    probabilities. See `codegen_priors.md` §3a Class 2b.
  - **(b) Normal-approximate (Gaussian-mixture) spike** — `β_j |
    γ_j = 0 ~ N(0, τ_0^2)` with a very tight `τ_0` (e.g. 1e-3),
    `β_j | γ_j = 1 ~ N(0, τ_1^2)`. Fixed-dim continuous relaxation;
    γ updated by `binary_gibbs_block`, β by `nuts_block` with the
...
2. **Coupling analysis.** See `codegen_cpp.md §4a`. **Default is JOINT:**
   ...
   block; if none fits AND NUTS is structurally inapplicable, `§2c`
...
4. **Ask if the user wants to review** (§3 above). If yes, show the
...
6. **Write one natural-scale log-density function per block.** This is
   ...
   never hand-roll Jacobians. See `codegen_cpp.md §6` for the template.
7. **Register the y_rep posterior-predictive stochastic refresher** for
   every block that has an observation layer. See `codegen_cpp.md §6a`
...
8. **Gradient verification (Check #12).** For every NUTS block with
   ...
   `tests_autodiff/verify_<ClassName>.cpp`. See `codegen_cpp.md §6b`.
...
11. **Verify (§11 below) — STRICT ORDER L1 → L2 → L3, no reversal.** Compile (L1), then the
    ...
    the **HARD ORDERING GATE** at the top of §11. Delete smoke-test file on PASS.
...
> file, `codegen_cpp.md §6b`). The L3 runtime harness is the **LAST** thing you build.
...
scripts. Same pattern as Check #12 verify files in `codegen_cpp.md §6b`.
...
- **Never generate a sampler whose target has a dimension-changing
  ...
  §3a` decision tree: propose Class 2a continuous spike / horseshoe;
  ...
  `normal_gamma_cluster_gibbs_block` + `nuts_block` on log α);
  for non-BNP unknown-K use `rjmcmc_block` with
  `examples/SpikeSlabRJMCMC.cpp` as template; or decline. See
  `skills/system_design.md` §11 for the measure-theory reason.
...
- **Always emit the GPL-3.0-or-later license header at the top of
  every generated `.cpp`** (see `codegen_cpp.md §5` for the exact
...
- **Never use runtime path detection in a generated R runner.**
  ...
  **Instead: emit HARDCODED RELATIVE PATHS** that the codegen agent
  ...
  See `codegen_r_runner.md §9` for the exact template.
- **Never expose per-parameter initial values as constructor arguments**
  ...
  prior hyperparameters — see `codegen_cpp.md §8` "Initial values" for
...
- **Always vectorize the natural-scale log-density via armadillo /
  ...
  (Apple Accelerate / OpenBLAS / MKL) and is 10-50× faster than a
  ...
  `// (N x p)(p x 1) = (N x 1)`). See `codegen_cpp.md §6.1` for the
  ...
  pattern ran ~11× slower than Stan's `bernoulli_logit_glm` reference.
- **The chain helper MUST be named `run_chain_<ClassName>()`
  ...
  `skills/codegen_r_runner.md §9 "R runner template"` for the
...
- **Always register a y_rep stochastic refresher.** `codegen_cpp.md §6a`
...
- **Always run Check #12 gradient verification at generation time.**
  For every NUTS block with a hand-written gradient, the AI must:
    ...
    4. **DELETE `tests_autodiff/verify_<ClassName>.cpp` after PASS,
       ...
       user's workspace is a Hard Rule violation (§12).
