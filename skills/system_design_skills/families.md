<!-- system_design MODULE — block-family conventions (extracted 2026-06-21 from system_design.md §13).
...
- **RNG.** Uses `readapt_rng_`, the third RNG stream separate from
  `rng_` (MCMC) and `predict_rng_` (predict_at). See §8. Same seed
...
- **Not available on VI / BART / Gibbs / RJMCMC / HMM / SBP / Slice
  ...
  readapt_NUTS). Tracked in §18.10 deferred-scope list.
- **Long-tail concentration / scale caveat.** For hierarchical
  models with heavy-tailed concentration parameters (Dirichlet
  α with Jeffreys prior, hierarchical SD τ, BNP mixture concentration),
  ...
  on `DirichletHierarchical` 2026-05-26 — α concentration's R-hat
...
- **Hybrid-composite stale-derived-state caveat.** `readapt_NUTS`
  ...
  **Workflow rule per composite type:**
  ...
  |---|---|
  | Pure NUTS-family only | `readapt_NUTS()` immediately ✓ |
  ...
  X. The σ `nuts_block` sibling's readapt adapter reads `f_bart` →
  uses stale residual `y - f_bart_stale`. Inserting one `step(1L)`
  between `set_current` and `readapt_NUTS` refreshes `f_bart`
  (BART's `step()` writes it back to shared_data), and σ's readapt
...
`validator.md §23` for full audit template.
...
**Sum-to-zero invariant.** ESS rotation `x cos θ + nu sin θ` is linear,
...
- N=16 ICAR 4×4 grid, pure-prior recovery: sample variance / Q^{-1}
  diagonal = 0.998 (essentially exact).
- N=16 Poisson-ICAR, 4 chains × 2k draws: max R-hat 1.026,
  coverage 15/16 = 94%, sum-to-zero invariant preserved to
  9.7e-13.
- N=64 Poisson-ICAR, 4 chains × 10k draws: max R-hat 1.041,
  coverage 63/64 = 98%, 0.09s wall per chain.

**Performance.** Per-step cost is dominated by sparse Cholesky
factorization of Q (O(n · b_w^2) with b_w ≈ O(√n) for 2D lattice
...
metric the step UNDERSHOOTS and mixing collapses (pl2: ~38× worse; 6-13× across
...
dense+single-pilot gave 5-23× higher keep-phase ESS/s than dense+3-phase (3-phase's
...
(8D equicorr ρ=.999 cond≈8000: single-pilot 0/8 vs 3-phase 8/8) — rare, usually a
...
ARMA's |θ|>1 ridge) is automatic on every metric path.
...
single-pilot FIXED it (1.0015, 6× ESS). Full C++ suite green after the flip.
...
- **Single-phase** (DEFAULT + RECOMMENDED; the field defaults `false`; original v1.1
  ...
  (corpus: 5-23× vs three-phase) and converges across all realistic cond.
...
- **Three-phase** (OPT-IN extreme-cond escalation only — slower in keep-phase; set
  ...
  failure): Stan-style windowed warmup (Carpenter et al. 2017, Stan ref manual §16.2):
  ...
  - **Phase II** (default `tp_phase2_windows = {25, 50, 100, 200,
    ...
    `Σ_reg = (n/(n+5))·Σ + 1e-3·(5/(n+5))·I`) as the new mass
...
three-phase warmup reduces wall-clock by roughly 2× on the same
...
Case study: see §15 (BART case study, lifecycle.md) for the full fix history (originally
...
`set_current(arma::vec)` writes the variational mean μ on the
unconstrained scale; log σ (mean-field) or vec(L) (full-rank)
keeps its current value. Tier A dispatcher routes per-component
keys `list(name_mean = μ_vec, name_log_sd = ls_vec)` (full-state
overwrite is rare — see §18.3).
...
q-SAMPLE (η ~ q, θ = T(η)) — NOT the q-mean — into shared_data
...
(VI optimizer = RAABBVI)** plus **the Layer-3 R2-VI PSIS-k̂
diagnostic (k̂ < 0.7)** are all mandatory before ship — see §18 for
