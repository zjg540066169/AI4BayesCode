<!-- system_design MODULE -- Variational Inference + rationale (extracted 2026-06-21 from system_design.md Sec.18-Sec.19).
...
invariants (Sec.Sec.1-17). It is the deep reference for the brief
family entry in Sec.13's "VI-family" subsection and for
...
prefer MCMC -- VI's bias (Sec.18.9) is not free.

### 18.1 Architectural insight -- VI factor == Gibbs block

Bishop Sec.10.1 derives mean-field variational inference as the
coordinate-ascent fixed point of the factorized family
`q(Z) = prod_i q_i(Z_i)`. The optimal q*_j satisfies
...
|---|---|---|
| State maintained | last sample theta-hat  in  Theta | variational params lambda = (mu, sigma) or (mu, L) |
| `step(rng)` does | one sample from p(theta\|others) | one optimizer step on -ELBO(lambda\|others) |
| `current()` returns | last sample | q-mean (point estimate) |
| What is written to shared_data | last sample | q-sample eta~q then theta=T(eta) |
...
- **`step(rng)`** runs ONE optimizer step (avgAdam from RAABBVI,
  Sec.18.7), not a sampling step. The rng is used inside `step()`
  for the reparameterization draw epsilon ~ N(0, I) and (optionally)
  data subsampling.
- **`current()`** returns the q-mean E_q[theta] ~= T(mu) (apply the
  natural-scale transform to the unconstrained mean mu). For most
  use cases this is what users want as a point estimate.
- **`set_current(mu)`** overwrites the variational mean ONLY; log
  sigma is left intact. To overwrite both, Tier A dispatcher routes
...
- **`current_sample(rng)`** is the new method invoked by the
  ...
  q-mean) into shared_data. See Sec.18.4 for the rationale.
- **`get_history()`** in Tier A wrapper, for any VI child, returns
  the ELBO + variational-parameter trajectory (per `step()` call,
  when `keep_history=true`). NOT posterior draws. See Sec.18.7 for
  the storage shape.
...
the Sec.6 working-response correctness invariant: the value that
...
| `m$predict_at(list(...))` from R | uses `predict_rng_` to draw q-samples (see Sec.18.7) |

### 18.5 Variational family -- mean-field PRIMARY, full-rank opt-in

**v1 primary: mean-field Gaussian on unconstrained R^K.**
Each user-declared parameter theta_i is wrapped via existing
`constraints::*::wrap`: the unconstrained eta_i = T^{-1}(theta_i)
lives in R, and

  q(eta) = prod_i q(eta_i) = prod_i N(eta_i; mu_i, sigma_i^2)

with variational parameters lambda = {(mu_i, log sigma_i)} (2K scalars for
a K-dim parameter). This is the Bishop Sec.10.1.3 univariate-Gaussian
...
conditional on hidden representations, hierarchical scale x parameter
funnels), the posterior marginal variance is collapsed to the
CONDITIONAL variance (Bishop Sec.10.1.2). The full-rank block computes

  q(eta) = N(eta; mu, L L^T)        L lower triangular

with variational parameters lambda = {mu, log L_diag, vec_lower_offdiag(L)}
(K + K(K+1)/2 scalars). Reparameterization gradient:
  eta = mu + L epsilon,   epsilon ~ N(0, I)
  d(-ELBO)/dmu_k       = -E_epsilon[(dlog p-tilde/deta_k)]
  d(-ELBO)/dL_ij (i>j) = -E_epsilon[(dlog p-tilde/deta_i) epsilon_j]
  d(-ELBO)/d(log L_jj) = -E_epsilon[(dlog p-tilde/deta_j) epsilon_j * L_jj] - 1
...
rho=0.95 bivariate Gaussian: mean-field collapses marginal sd to
sqrt(1-rho^2)=0.31 (conditional variance), full-rank recovers marginal
sd ~= 1.0 AND correlation ~= 0.95, with PSIS-k-hat ~= -0.14 (q ~= p exact).
Same correctness signature on the BNN regression hyperparams
(sigma_y/sigma_alpha/sigma_beta reach R-hat<1.20 vs NUTS-truth, vs mean-field's R-hat>1.36
on sigma_alpha and >1.51 on sigma_beta).

**No new Jacobian discipline (Sec.10).** `constraints::*::wrap`
...
hand-written `+ log(sigma)`) applies unchanged.

### 18.6 Objective -- exclusive KL, reparameterization gradient

**Default objective: exclusive KL** (ELBO maximization):

  -ELBO(lambda) = E_{eta~q(*;lambda)}[ ln q(eta; lambda) - ln p-tilde(eta) ]

where p-tilde(eta) = p(theta=T(eta), y) * |det dT/deta| is the natural-scale
...
**Gradient via reparameterization trick:** for mean-field Gaussian,
  ...
  eta_i = mu_i + sigma_i * epsilon_i,   epsilon_i ~ N(0, 1)
  grad_mu_i  [-ELBO]      = - E_epsilon[ grad_eta_i ln p-tilde(eta) ]
  grad_logsigma_i [-ELBO]    = - E_epsilon[ epsilon_i * sigma_i * grad_eta_i ln p-tilde(eta) ] - 1

(Kucukelbir et al. 2017, ADVI eqs. 4-6). The inner
grad_eta_i ln p-tilde is exactly the user's existing natural-scale gradient
...
computing E_q[f(theta)] for any posterior expectation (predictive,
LOO, BPV), draw S samples eta_s ~ q, compute importance weights
w_s = p-tilde(eta_s) / q(eta_s), apply PSIS to stabilize w_s, then take
...
1. **avgAdam**: like Adam, but the running squared-gradient
   ...
   averaging for the final lambda-bar_gamma estimate.
2. **R-hat-based convergence detection at fixed gamma**: within each
   learning-rate epoch gamma_k, treat iterates as a single chain and
   compute `R-hat_max(W) < 1.1` for an adaptive window
   W  in  [W_min, 0.95*k]. On detection, declare epoch convergence.
3. **Geometric gamma decay**: gamma_{k+1} = rho * gamma_k between epochs
   (default rho = 0.5, initial gamma_0 = 0.1).
4. **SKL inefficiency-index termination**: I = RSKL x RI < tau
   stops the entire optimization. RSKL = symmetric KL between
   successive averaged iterates lambda-bar_{gamma_k} and lambda-bar_{gamma_{k-1}} (closed
   form for Gaussians), RI = relative inefficiency (variance of
   lambda-bar_gamma estimate vs Monte Carlo bound), default tau = 0.1.
...
### 18.8 Layer-3 diagnostic -- PSIS-k-hat replaces R-hat for VI blocks
...
**v1 ships joint Pareto-Smoothed Importance Sampling k-hat** (Yao
et al. 2018, Dhaka 2021) as the VI-block Layer-3 R2 substitute:

1. Draw S samples eta_s ~ q (default S = 1000).
2. Compute log-importance-weights log w_s = log p-tilde(eta_s) - log q(eta_s).
3. Fit a generalized Pareto to the upper tail of w_s; the shape
   parameter k-hat is the diagnostic.
4. **Joint** (not marginal): k-hat is computed on the full eta vector,
   not per parameter. Marginal k-hat misses joint failure modes
   (Yao et al. 2018).
...
| >= 0.7 | FAIL | retry up to 5 attempts per `start.md` failure-recovery policy: try full-rank on largest |k-hat_marginal|, switch to non-centered reparam, increase optimizer epochs, drop tau to 0.05, init from multiple seeds and pick best ELBO |

**Hybrid mode Layer-3 R2**: split the diagnostic by child kind:
MCMC children get R-hat (< 1.05), VI children get PSIS-k-hat
(< 0.7), plus an aggregated **ELBO trajectory plot** rendered
inline in the runner R script. There is intentionally no joint
hybrid convergence criterion in v1 (Sec.18.10).

**Layer-3 R2-VI PSIS-k-hat diagnostic** (a Layer-3 runtime check, NOT a
numbered semantic check): PSIS-k-hat < 0.7 for every VI-family block.
Layer-3 fails fast with the action checklist above on k-hat >= 0.7. See
`validator.md Sec.R2-VI` for the gating R code (3-tier verdict, CAUTION
...
4. **High dim != automatic VI win.** Dhaka 2021 showed VI in
   ...
   is bad either way. The v1 defaults (Sec.18.6, Sec.18.7) are the
...
- **Joint convergence criterion for hybrid.** v1 runs hybrid
  ...
  per-child diagnostics (R-hat for MCMC, k-hat for VI, ELBO
...
- Bishop, C. M. (2006). *Pattern Recognition and Machine Learning*,
  Sec.10.1 (modular variational inference).
...
- **BART set_current(X, y) support (2026-04-19).** See Sec.15
  case study.
