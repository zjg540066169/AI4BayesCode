...
| `target_accept_rate` | tunable | 0.55 (default) | AI4BayesCode default 0.55 (vendored mcmclib default; Hoffman-Gelman 2014). Behaves like Stan/PyMC `adapt_delta`: raising it (0.8-0.99) takes smaller steps + more leapfrog work per iteration, which helps stiff funnel / hierarchical / divergent geometries -- the normal Stan tradeoff. (NOTE: through 2026-08-09 a dual-averaging accept-stat bug -- the reference H0 drifted to the current sample instead of the fixed trajectory start, deviating from HG2014 Alg 6 -- made raising the target COLLAPSE the step size to max-tree-depth; FIXED 2026-08-10. Raising is now safe.) Prefer 0.55; raise like adapt_delta when a model diverges or mixes poorly. Also settable per readapt() call. |
