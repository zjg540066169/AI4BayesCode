...
| `target_accept_rate` | YES (rare) | 0.5 - 0.99 | AI4BayesCode default 0.55 (matches Hoffman-Gelman 2014 and the vendored mcmclib default). Stan / PyMC / NumPyro use 0.8; pass 0.8 here to match their behaviour on a per-block basis. Raise to 0.9 - 0.99 for stiff funnel / hierarchical models where divergences persist. Leave at 0.55 unless a specific model demands otherwise (raising the wrapper default to 0.8 introduced a hierarchical funnel mixing regression on a stress model in earlier testing). Also settable per readapt() call via the `readapt_NUTS(..., target_accept)` 4th arg. |
