...
anchored-softmax parameterisation `eta_i  in  R^{K_i-1}` and `phi_{i,0}`
anchored to `1 - Sigma_{l>0} phi_{i,l}`. Optimises `-ELBO` via RAABBVI on
the analytically-correct gradient `dELBO/dphi_{i,k} = G_{i,k} - log
phi_{i,k} - 1` where `G_{i,k} = E_{q\i}[log p-tilde(z_{-i}, z_i = k)]`.

**Two gradient modes** (selected by `cfg.exact_enumeration`):
- **Exact**: enumerate full joint state space `prod_i K_i` (capped at
...
- **Monte Carlo** (default): sample `S = cfg.n_mc_samples` joint
  ...
  evaluate `log_density`. Cost = `S x Sigma_i K_i` evaluations per step.

**JUSTIFICATION (Check #16)**: Discrete latents with strong local
dependence (`system_design.md` Sec.11.2(b)). Per-site Gibbs irreducible
...
| Joint PSIS-k-hat on coupled chain | 5.78 (FAIL -- correctly diagnoses MF bias) |

**Public interface** (`vi_block` contract):
- `current()` returns concatenated phi (length `Sigma_i K_i`) -- the q-mean
  point estimate
- `current_sample(rng)` returns one z draw (length `n` integer
  indices in `{0..K_i-1}`)
- `get_log_sd()` returns the unconstrained eta vector (length `Sigma_i (K_i-1)`)
- `set_current(phi_concat)` accepts a probability vector and recovers
  eta via inverse softmax
...
- Continuous-relaxation alternatives (Concrete / Gumbel-softmax) --
  explicitly rejected by plan Sec.4 (analytical-KL is more accurate)
