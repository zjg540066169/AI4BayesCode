...
3. **Joint NUTS over `(amp, ell)` -- DEFAULT in `GPClassification.cpp`.**
   ...
   (POSITIVE x 2) so the two hyperparameters are sampled together
   under a shared mass matrix. If you split them back into separate
   `nuts_block`s for any reason, expect 5-10x ESS loss on `amp`.
...
5. **Switch the joint log-density to an analytic gradient.** The
   ...
   noise-free gradient at O(N^3) one-shot + O(N^2) per theta-component;
