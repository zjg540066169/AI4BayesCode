...
# Writes results to audit_results_5k5k.rds.
...
c1 <- run(101L, 5000L, 5000L); c2 <- run(202L, 5000L, 5000L)
...
loo_ <- loo_from(LL1, LL2)
# r recovery (posterior mean vs truth)
...
c1 <- run(101L, 5000L, 5000L, rep(0,N_i), rep(0,J_i), 1.0)
c2 <- run(202L, 5000L, 5000L, rnorm(N_i,0,0.5), rnorm(J_i,0,0.5), 0.5)
d_theta <- diag_arr(pack2(c1$theta, c2$theta))
...
# Save + emit summary
# ============================================================================
saveRDS(results, file.path(AI4BayesCode_dir, "audit_results_5k5k.rds"))
cat("\n=== SUMMARY ===\n")
