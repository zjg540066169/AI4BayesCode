# Phase C2 of audit — MCMC diagnostics on all 11 production examples,
# long-chain budget: 5000 burnin + 5000 keep per chain × 2 chains.
# Tests whether the short-chain warnings (DirichletSparse,
# DirichletHierarchical, BART pareto-k) were chain-length-dependent.
# Writes results to audit_results_10k10k.rds.
...
# ============================================================================
cat("\n[C.1] GaussianLocationScale\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "GaussianLocationScale.cpp"),
...
c1 <- run(101L, 10000L, 10000L); c2 <- run(202L, 10000L, 10000L)
...
# ============================================================================
cat("\n[C.2] BetaBernoulli\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "BetaBernoulli.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.3] DirichletSimplex\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "DirichletSimplex.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.4] DirichletSparse\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "DirichletSparse.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.5] DirichletHierarchical\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "DirichletHierarchical.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "BartNoise.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.7] GBartPoisson\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "GBartPoisson.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.8] ARDLasso\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "ARDLasso.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "IRT1PL_joint.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
c1 <- run(101L, 10000L, 10000L, rep(0,N_i), rep(0,J_i), 1.0)
c2 <- run(202L, 10000L, 10000L, rnorm(N_i,0,0.5), rnorm(J_i,0,0.5), 0.5)
d_theta <- diag_arr(pack2(c1$theta, c2$theta))
...
# ============================================================================
cat("\n[C.10] HierarchicalLM_joint\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "HierarchicalLM_joint.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
cat("\n[C.11] LinearRegJointMixed\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "LinearRegJointMixed.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
...
# ============================================================================
# C.12 SpikeSlabRJMCMC (Ishwaran-Rao sigma-scaled slab + double Jeffreys)
# ============================================================================
cat("\n[C.12] SpikeSlabRJMCMC\n")
ai4bayescode_source_checkout(file.path(AI4BayesCode_dir, "examples", "SpikeSlabRJMCMC.cpp"),
                    AI4BayesCode_path = AI4BayesCode_dir)
set.seed(1); N_ss <- 150L; p_ss <- 20L
X_ss <- matrix(rnorm(N_ss*p_ss), N_ss, p_ss)
X_ss <- scale(X_ss, center=TRUE, scale=FALSE)
beta_t_ss <- c(2.5, -1.8, 1.2, rep(0, 17))
sigma_t_ss <- 1.0
y_ss_raw <- as.numeric(X_ss %*% beta_t_ss + rnorm(N_ss, 0, sigma_t_ss))
y_ss <- y_ss_raw - mean(y_ss_raw)
run_ss_c <- function(seed, nb, nk) {
    m <- new(SpikeSlabRJMCMC, X_ss, y_ss, 1, 1, as.integer(seed), TRUE)
    t0 <- Sys.time(); m$step(nb); m$step(nk); t1 <- Sys.time()
    h <- m$get_history()
...
c1 <- run_ss_c(101L, 10000L, 10000L)
c2 <- run_ss_c(202L, 10000L, 10000L)
active_idx <- c(1, 2, 3)
gamma_all <- rbind(c1$gamma, c2$gamma)
beta_all  <- rbind(c1$beta,  c2$beta)
incl <- colMeans(gamma_all)
d_g_act <- diag_arr(pack2(c1$gamma[, active_idx, drop=FALSE],
                          c2$gamma[, active_idx, drop=FALSE]))
d_b_act <- diag_arr(pack2(c1$beta[, active_idx, drop=FALSE],
                          c2$beta[, active_idx, drop=FALSE]))
d_sig   <- diag_arr(pack2(c1$sigma, c2$sigma))
d_tau   <- diag_arr(pack2(c1$tau,   c2$tau))
d_pi    <- diag_arr(pack2(c1$pi,    c2$pi))
beta_cond <- numeric(length(active_idx))
for (k in seq_along(active_idx)) {
    j <- active_idx[k]
    act_rows <- which(gamma_all[, j] == 1)
    if (length(act_rows) >= 100)
        beta_cond[k] <- mean(beta_all[act_rows, j])
    else
        beta_cond[k] <- NA
}
rec_s_ss <- summarize_recovery(c(c1$sigma, c2$sigma), sigma_t_ss)
results$SpikeSlabRJMCMC <- list(
    wall = c1$wall + c2$wall,
    d_gamma_active = d_g_act, d_beta_active = d_b_act,
    d_sigma = d_sig, d_tau = d_tau, d_pi = d_pi,
    incl = incl, beta_cond = beta_cond, beta_true_active = beta_t_ss[active_idx],
    rec_sigma = rec_s_ss$within_3sd, post_sigma = rec_s_ss$post_mean,
    min_tpos = min(incl[active_idx]),
    max_fpos = max(incl[-active_idx]))
cat(sprintf("  wall=%.1fs  gamma_active Rhat=%.3f  beta_active Rhat=%.3f\n",
    results$SpikeSlabRJMCMC$wall, d_g_act$rhat_max, d_b_act$rhat_max))
cat(sprintf("  sigma Rhat=%.3f  tau Rhat=%.3f  pi Rhat=%.3f\n",
    d_sig$rhat_max, d_tau$rhat_max, d_pi$rhat_max))
cat(sprintf("  sigma pm=%.3f (true=%.3f)  min_tpos=%.3f  max_fpos=%.3f\n",
    rec_s_ss$post_mean, sigma_t_ss,
    results$SpikeSlabRJMCMC$min_tpos,
    results$SpikeSlabRJMCMC$max_fpos))
cat("  per-signal beta_cond:\n")
for (k in seq_along(active_idx)) {
    j <- active_idx[k]
    cat(sprintf("    j=%d  incl=%.3f  beta_cond=%.3f (true=%.2f)\n",
        j, incl[j], beta_cond[k], beta_t_ss[j]))
}

# ============================================================================
# Save + emit summary
# ============================================================================
saveRDS(results, file.path(AI4BayesCode_dir, "audit_results_10k10k.rds"))
cat("\n=== SUMMARY ===\n")
