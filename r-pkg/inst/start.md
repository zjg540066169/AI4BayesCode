...
- If the mode is NOT clear (e.g. only "read start.md", or ambiguous), pop ONE routing
  ...
  environment-appropriate mechanism (AskUserQuestion in Claude Code; the §0 markdown
  fallback otherwise):
...
- **(a) Generate a sampler** → the standard flow: continue with the §1 phase schedule
...
code is done before L1 + L2 + L3 R1/R2/R3 all pass. See §5.
...
- **Mark the standing / suggested choice as `(default)`** in its
  label — e.g. `"Weakly informative N(0, 5²) (default)"`. NEVER label
...
| Render the prediction DAG inline (Mermaid / edge list) for the §3 step | NO |
...
| Load the next instruction file per the phase schedule in §1 | NO |
...
request_user_input / markdown fallback per §0 above).
...
| 1. Upfront questions | after reading this file | `skills/codegen.md` §1 |
| 2. Prior elicitation | when a user prior is missing / ambiguous | `skills/codegen_priors.md` |
| 3. Model confirmation | before writing any code | `skills/codegen.md` §2 (inline DAG + summary table) |
...
line, no absolute path. See `codegen_cpp.md` §5.

---

## 2. Output folder convention

The output folder comes from the upfront questions (§1 phase 1).
...
`codegen.md` §2(b) for the exact templates.
...
sampling". See `codegen.md` §2(b) MANDATORY note.

## 3b. Math: DISPLAY-block LaTeX for equations; Unicode for inline symbols
...
- **PREFER display blocks for ALL real math** — it renders and reads best (noticeably
  better than Unicode), so when in doubt, lift the formula into a `$$ ... $$` block rather
  than inlining it. A multi-line `\begin{aligned}` block is the default for any model /
  likelihood / prior set.
- **Unicode is the NARROW fallback** — use it ONLY for a lone symbol mid-sentence (β, σ, ν,
  rᵢ) where a display block would break the prose AND inline `$...$` won't render. Never
  render a whole equation as Unicode if it could go in a block.
- **NEVER** inline `$...$` for anything you want rendered — it shows the raw source here.
- **Tables are the trap** (the most common slip). A markdown table cell CANNOT hold a
  `$$...$$` display block (it breaks the row), and inline `$...$` inside a cell shows raw.
  So **inside any table cell, use plain Unicode ONLY** — here Unicode is MANDATORY, not a
  fallback. Common glyphs: `X y β σ ν μ Σ Γ ∇ ℝ × ≥ ≤ ∞`, transpose `Xᵀ`, sub/superscripts
  `rᵢ ℝᵖ xᵢ`, `n×p`, `>0`, `ℝᵖ`. So the screenshot's `$X$ / $\beta$ / $\mathbb{R}^p$ / $>0$ /
  $n\times p$` cells must be `X / β / ℝᵖ / >0 / n×p`. If a cell's math is too complex for clean
  Unicode, do NOT force it into the table — lift it into a `$$...$$` block ABOVE the table and
  keep the cell to a short Unicode label.
- **Inline expressions in prose** follow the same split: a whole expression (e.g.
  ∇_β log p = Xᵀ W r, or Wᵢ = (ν+1)/(νσ² + rᵢ²)) goes in a `$$...$$` block; only a lone symbol
  stays inline as Unicode. Never inline `$...$` an expression mid-sentence.

(DAGs stay inline per §3 above.)
...
## 5. Hard rules (from `codegen.md` §12)

These apply at every phase; you may skim `codegen.md` §12 for the
...
- **Block-selection priority** (TWO-PHASE, not a linear "first item wins"):
  ...
  **`joint_nuts_block` is the DEFAULT** for every continuous parameter left →
  ...
  resort. See `codegen_priors.md` §2b (authoritative).
- **No Mermaid PNG render**: see §3 above.
...
- **Every generated `.cpp` carries a runnable `@example:<backend>` block
  ...
  the standalone `int main()` (see `codegen_cpp.md` §5 "Header `@example`
...
3. **Once the user sends the model description** (sampler branch), load
   ...
   Max generate→validate attempts? (codegen.md §1.7)
   (a) 5 (default)
   (b) 10
   (c) 20
   (d) Unlimited (opt-in)
   ...
   **Forbidden formats — every one is a bug** (the user can't
   ...
   - Prose paragraph listing the questions: ❌ "Please provide:
     runtime backend, output folder, class name, AI4BayesCode
     path, sampler preference, max attempts."
   - Comma-separated defaults: ❌ "Standing defaults are: R,
...
4. After the user answers, proceed phase-by-phase per §1 above.
