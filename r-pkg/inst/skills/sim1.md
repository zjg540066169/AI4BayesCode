...
Build a list-of-(iter x dim) matrices for every parameter that is
...
index and bind into an iter x dim matrix:
...
`label_switching.md Sec.3 (LDA / topic model)` for the explicit formula
...
1. **AI4BayesCode `get_history()` shape** -- scalar parameters MAY come
   back as numeric vectors (length = iter) or as `iter x 1` matrices.
