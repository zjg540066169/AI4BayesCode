...
# (license / attribution), validate.md (the stateful-compatibility check), system_design Sec.1 (the block
...
`00_flow.md Sec.0.5`. **This module is the ENGINEERING:** making borrowed code correct inside AI4BayesCode's
STATEFUL block contract (`system_design Sec.1`: `step(rng)` / `set_context` / `current` / `set_current`,
...
- **Upstream LICENSE header stays VERBATIM** (`intake.md` Step 5): never relicense the vendored source.
  YOUR adapter code is GPL-3.0-or-later; the vendored source keeps its own license. (`Sec.5c` verifies both.)

## 2. State audit -- find the state FIRST

Before adapting, find every piece of state the kernel holds -- the same four the VALIDATE
stateful-compatibility check will test (`validate.md` Sec.1 vendored-kernel check):
...
## 5. What VALIDATE then checks (your Sec.3 work is what makes it pass)

For a vendored kernel, VALIDATE runs BOTH:
- the **vendored-correctness regime** -- the routine standalone vs KNOWN values (`validate.md` Sec.1), AND
- the **vendored-kernel STATEFUL-compatibility check** (`validate.md` Sec.1 / Sec.2) -- no hidden global state,
  determinism under the block's `rng`, cache rebuild on `set_context`, instance isolation.

Your Sec.3 modifications are exactly what make that check pass. The `Sec.5c` final license check then confirms
