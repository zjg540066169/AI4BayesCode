...
| **dense mass metric** used by the block | **Check #18** (dense metric justification + pilot scaling): dense is **diagnostic-driven escalation** -- start diagonal, MEASURE, escalate on evidence; it is NOT a dimension threshold. Requires a plain-language `// Sampling note: ...` in the block/example plus the measured evidence recorded in the L2 verdict table (check numbers and diagnostics never ship in a delivered file). |
