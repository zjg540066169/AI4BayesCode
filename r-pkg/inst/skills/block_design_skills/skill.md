...
<!--
 ...
 Key sections: Sec.1 (skill delimitation), Sec.3 (manifest.dcf), Sec.4 (SelectWhen / two-stage selection).
...
1. `skills/<Block>.md` -- the **block-local skill**: its catalogue entry (routing / when-to-use /
 ...
 block-SPECIFIC `BL#` checks do NOT go here** (Sec.2.2) -- they go in artifact 3.
...
the markdown labeled-option fallback from `start.md Sec.0`); the standing choice is marked
...
the manifest `SelectWhen` (Sec.4 below); keep them identical so Stage-B selection and the
...
**(d) Geometry class** -- which of `system_design Sec.11.1`'s three shapes the target is:
fixed-dim R^d (after constraint transforms) * simplex * trans-dimensional/Sec.11.2. State it in
...
- `TargetGeometry` -- coarse; the flow's geometry gate already pre-routes (geometry is named
 in the block-local skill Sec.2.1(d) for humans, not duplicated as a manifest field).
- `ClassName`, `Header` -- derived 1:1 from `Block` (enforced convention, Sec.3 above).
...
**Identity rule:** the skill's WHEN-to-use (Sec.2.1(b)) and the manifest `SelectWhen` are the
...
Sec.2b`'s wording is synced to "auto-select + confirm at the gate" -- out of scope for this phase,
...
- [ ] No paragraph in the block-local skill restates a core convention (delimitation rule Sec.1).
...
- [ ] Name-uniqueness + license-gate + SelectWhen-quality re-confirmed (Sec.3.5).
...
- [ ] **BL# 3-way consistency.** The `BL#` defined in the `ValidationSkill` (or Sec.2.2) == the `BL#`
...
- [ ] **Paths + cross-refs resolve.** `Skill` / `Tests` / `Example` / `ValidationSkill` files exist
  on disk; every `system_design Sec.`, `validator.md #`, and file path the docs cite actually exists.
...
(T0-T4), AFTER the EXAMPLE phase, and AFTER the Sec.5b doc-consistency checks, immediately before the
auto-delivery move into `blocks_local/`.** (It is the final check; passing it is what lets the
automatic move proceed.) It closes the loop with the `00_flow.md Sec.0.5` GPL-3 gate the user agreed to at the
...
T0-T4 + Check #12 + any vendored-correctness / stateful checks; the example if authored; the Sec.5b
doc-consistency; the Sec.5c GPL-3 license check), MOVE the whole bundle from staging into
