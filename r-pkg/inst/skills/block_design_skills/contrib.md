<!-- block_design MODULE — contrib.md — CONTRIB ARCHITECTURE reference (load on demand).
 ...
 CITE, don't restate: per-block skill split → skill.md §1; manifest fields → skill.md §3;
...
- The name is the unique key and the single source of identity — `ClassName`, header
 ...
 the manifest `Block:` field conventions, `skill.md §3`).
- Flat naming carries typosquatting/grab risk, mitigated by the manifest's `Author` /
 provenance record (+ trust badges [FUTURE]).
- **block_design checks uniqueness AT CREATION** against `core ∪ existing local` (this is the
 ...
 **on demand alongside the block code** — never preloaded (token discipline; see skill.md §4,
...
model-confirmation summary (`start.md` Stage 3 / `codegen.md` §2 — inline DAG + summary table +
...
This **refines** `codegen_priors.md §2b`'s older "block-selection disambiguation → ask the user
(mid-flow)" into "**auto-select + confirm at the one model-confirmation gate**" (one
consolidated gate, not scattered prompts). When this flow is wired in, KEEP §2b's wording in
...
at the source keeps the shared pool curated. (Gate and retrieval §4 are complementary: the gate
...
~10000× slower on BYM2-shape targets):
...
CITES them, does not restate. For the full local-vs-core delimitation table see **skill.md §1**,
and for the discovery/loading rule see **skill.md §4** — do not duplicate them here.
