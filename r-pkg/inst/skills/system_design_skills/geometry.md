<!-- system_design MODULE — target geometry + memory (extracted 2026-06-21 from system_design.md §11-§12).
...
- **Gibbs on hybrid space (`binary_gibbs_block` + "fix β=0 when γ=0")**
...
- **NUTS on the γ-marginalized density** is **silently wrong**.
  Marginalizing γ leaves
...
construction, so §10.2's universal rule "users never write a
Jacobian" holds). Library 1D transforms (linear / affine ported
from librjmcmc) plus runtime autodiff on user-supplied
bijections) will extend coverage; see §10.2 for the Jacobian
...
- `gmrf_precision_block` (SHIPPED 2026-05-31) covers Gaussian MRFs
  with sparse precision Q (Rue 2001 §2 + §3.1.2 + §3.1.3 simplified
...
- `order_mcmc_block` (SHIPPED 2026-05-31) covers Bayesian-network
  ...
  variables, FK §4.2 three-tier cache) via Friedman-Koller 2003
  ...
  * `tests/audit_OrderMCMCBN.R` (30/30 PASS — Layer-3 plateau,
    STRICT R-hat < 1.01 (Vehtari 2021), §16 R-level pre-merge
  ...
  * `tests/test_order_mcmc_block_diagnostics.cpp` — **exact
    ...
    posterior (HARD max |Δ| < 0.05, achieves ≈ 0.001). D2 n = 4
  ...
  * `tests/audit_OrderMCMCBN_vs_BiDAG.R` — **reference-
  ...
  data + BDeu only; documented FK §4.1 induced-structure-prior
...
2. **Check the support of every variable.** If any variable's
   ...
   state space is not a fixed-dim manifold), you are in §11.2(a).
...
3. **Check for strong local dependence** among discrete variables.
   If yes, you are in §11.2(b). Do NOT assume per-site Gibbs is
   enough. Either:
...
4. **Document the target class in the new block's header comment.**
   Every block header should state which of §11.1's three shapes
...
`skills/codegen_priors.md §3a` carries the decision tree (Class 1-5)
and `skills/codegen.md §12` carries a Hard Rule pointing here. The
code-gen agent's only obligation is: if a user
model description implies §11.2(a) or §11.2(b), the agent falls
...
scale-invariant Jeffreys prior p(sigma) ∝ 1/sigma,** implemented via
`nuts_block` + `constraints::positive::wrap` on log(sigma). On the
unconstrained η=log(sigma) scale, Jeffreys + Jacobian exactly
...
**PROHIBITED as a default "noninformative":** InverseGamma(ε, ε)
with small ε. Gelman (2006) Bayesian Analysis 1(3):515-533 §3
shows this prior has highly unstable behavior as ε → 0 and posterior
...
keep `p(param) ∝ 1/param` as the main prior; inside the natural-
...
half-Normal(0, A) with A ≈ 2.5 × data-scale as a persistent proper
...
identifiability issue in `y = BART(X) + N(0, sigma²)`). Document
the source.
...
A ≈ 2.5 × data-scale. Do NOT reach for IG unless you can justify
it in writing. See `skills/codegen_priors.md §2a` for the precise recipe.
...
- **Check #15** — library-level OR per-usage parity test present
  (see `skills/codegen_priors.md` §2c). Library-level test
...
- **Check #16** — inline "JUSTIFICATION (Check #16): Exception N
  from codegen_priors.md §2b — ..." comment above every gibbs usage stating
