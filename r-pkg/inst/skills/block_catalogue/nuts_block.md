...
| `target_accept_rate` | tunable, NOT recommended | 0.55 (default) | AI4BayesCode default 0.55 (Hoffman-Gelman 2014 + the vendored mcmclib default). Raising it is an OPTION but STRONGLY DISCOURAGED: unlike Stan/PyMC `adapt_delta` (where raising is safe and helps hard geometries), on THIS kernel raising the target tends to lengthen NUTS trajectories dramatically -- measured ~100x slowdown, near-max-tree-depth every iteration, on a GP-hyperparameter geometry at 0.8 vs ~9 leapfrog/iter at 0.55 (0.8 also once caused a hierarchical funnel mixing regression). So do NOT reflexively transfer the Stan/PyMC habit of raising adapt_delta for stiff funnel / hierarchical / divergent models -- it is REVERSED here. Prefer 0.55; raise only with measured evidence it helps THIS model. Also settable per readapt() call. |
