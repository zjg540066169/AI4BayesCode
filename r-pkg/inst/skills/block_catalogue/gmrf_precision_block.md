...
(Rue eq. 4 / Sec.3.1.2):
  ...
  pi(x) prop.to exp{ -1/2 x^T Q x + b^T x },   x ~ N(Q^{-1} b, Q^{-1})
...
sparse precision (`system_design.md` Sec.11.1 class 1, specialised
...
- Single sum-to-zero constraint via post-hoc projection on a
  ridge-regularised Q (simplified Rue Sec.3.1.3, the spam / R-INLA
  approach)
- AMD reordering via Eigen built-in

**Deferred to v1.2.1:**
- Arbitrary linear constraints A x = b (Rue Sec.3.1.3 exact kriging)
- METIS reordering for very large graphs
- Conditional sampling x_A | x_B (Rue Sec.3.1.1)
- [OK] **Knorr-Held & Rue (2002) joint block update of hyperparam + x --
...
- Divide-and-conquer for very large GMRFs (Rue Sec.4)
