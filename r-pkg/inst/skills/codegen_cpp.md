...
deliberate isolation. Coupled continuous parameters mix ~10× slower per
...
inapplicable, see `codegen_priors.md §2c` Exception 4.)
...
| Pólya-Gamma augmented logistic (conjugate after augmentation) | Low | **`pg_logistic_block`** — 10-100× faster than NUTS-on-logistic for p < 1000. **LINEAR LOGISTIC ONLY — NOT for logistic BART** (use `genbart_block + logistic_lik` instead; PG breaks BART's tree-scale identifiability). |
| Poisson / NB / heteroscedastic / AFT / beta / gamma_shape BART | Medium | **`genbart_block` with the matching `genbart::lik::*`** (Linero 2022 RJMCMC with Laplace leaf proposals; the single primitive handles all 10 shipped likelihoods + user-supplied). Reference examples shipped: `examples/GBartPoisson.cpp` (count regression), `GBartHeteroscedastic.cpp` (mean = variance = exp(r)). For NB / AFT / beta / gamma_shape / beta_binomial, the agent should compose from `GBartPoisson.cpp` + the corresponding `genbart::lik::*` likelihood header in `bart_pure_cpp/src/GENBART/likelihoods/`. |
| Bernoulli BART (binary classification) | Medium | **`genbart_block + logistic_lik`** directly (no augmentation) — see `examples/GBartLogistic.cpp`. Do NOT use `pg_logistic_block + bart_block` — PG's κ = y − 0.5 pseudo-response breaks BART's Gaussian-observation tree-scale identifiability. |
| Multinomial logistic BART (C >= 2 classes) | Medium | **C-1 × `genbart_block(poisson_lik, offset_key="log_phi_aug")` + one `poisson_multinomial_aug_block`** via the Baker 1994 / Forster 2010 gamma trick in the Murray 2021 §3.1 reference-category identified architecture. See `examples/GBartMultinomial.cpp`. For binary (C=2) prefer the simpler `GBartLogistic` direct path. |
| Dirichlet-Categorical conjugate | Low | `dirichlet_gibbs_block` + `categorical_gibbs_block` |
| GP regression with **Gaussian** observation likelihood | (no latent f sampled) | **Marginal-likelihood architecture**: integrate f out, sample only `(amplitude, lengthscale, sigma)` from `y ~ N(0, K + σ² I)` via one `joint_nuts_block` (3 POSITIVE slices). Analytic gradient via Rasmussen & Williams §5.5 Eq. (5.9) `0.5 tr((α α' − K⁻¹) ∂K/∂θ)`. NO ESS, NO latent f or z. See `examples/GPRegression.cpp`. Matches Stan / libgp / GaussianProcesses. |
...
| **DPMM with split-merge acceleration (mode-escape via Jain-Neal 2004)** | Cluster partition has slow single-i Gibbs mixing | Add `split_merge_block` as a child AFTER `categorical_gibbs_block` in any DPMM composite (DP / PY / Finite). Both write to `z`; composite allows multi-children writing the same key. See `block_catalogue.md` `split_merge_block` § for details and acceptance asymmetry note. |
...
4. A standalone `nuts_block` is the **LOW-priority** fallback: a
   ...
   isolation.
5. **Structural gap (`codegen_priors.md §2c` Exception 4):** if — judged
...
### Coupling outside the table (no §4a match)

When §4a lists no matching pattern, the DEFAULT is still ONE
...
`codegen_r_runner.md` and `validator.md §R2`). Do NOT use
...
3. **Enable dense metric only when** ONE of:
   ...
   - A documented diagonal-failure case applies. Currently three
     ...
     1. GP banana ridge in heteroscedastic / hierarchical / multi-
        output GP per `codegen_priors.md §11.4` + `block_catalogue.md`
     ...
     3. **Multivariate hierarchical** (large `G*d`) per the §4a coupling
...
5-23× higher keep-phase ESS/s than three-phase and converges across all realistic
...
step ~38×). NOTE (2026-06-20 flip validation): the shipped HSGP example's
three-phase path was actually BROKEN (R-hat 1.017) and single-pilot FIXED it
(1.0015, 6× ESS). See system_design.md §13.
...
- **Emit ONLY the block(s) for the runtime backend chosen in §1.** R → just
...
### Joint NUTS log-density template (the default path; see §4a)
...
double joint_theta_b_log_density(const arma::vec& theta_cat,
    ...
    // (5) compute likelihood and gradient via BLAS per §6.1; the slice
    //     convention in (4) applies to where the gradient is ASSIGNED,
    //     NOT to how it's computed. See §6.1 "Joint case" for the
...
which is 10-50× faster than a hand-rolled scalar inner loop on the
exact same math.

The cost of a slow gradient is multiplicative across the whole MCMC:
each NUTS step makes O(10-100) gradient calls. A 10× slow gradient =
10× slow chain. Concrete observed gap on a small (N=80, p=7)
Bernoulli logistic regression: scalar-loop AI 45.9s vs Stan
`bernoulli_logit_glm` 4.2s, an 11× gap that is entirely from this rule
being violated.
...
for typical GLM dimensions (N ≤ 10⁴, p ≤ 10²).
...
linear model with `g_idx[n] ∈ {0..G-1}` mapping observation n to
...
no cache blocking. ~10-50× slower than the `X * theta_nat` /
...
NUTS iterations × ~500 leapfrog steps that's tens of millions of
...
inline arma::vec log_inv_logit_arma(const arma::vec& z) {
    return -softplus_arma(-z);                           // log(σ(z))
}
```
...
**2. Add the mutable predict RNG member** to the wrapper class (see §8).
...
unit" is the count vector, so LL has shape `iter × 1`.
...
5. **NEVER collapse intermediate deterministic latents into the y_rep
   ...
   keys ∪ values, ∪ `data_inputs`, MUST equal the node set of the
   user-verified DAG (modulo the `y_rep` renaming of the observed
   `y` node). See §6c below for the canonical multi-layer pattern
...
1. Topology is **read from the model's own prior code, never
   ...
   `sigma^2 ~ IG(nu/2,nu·λ/2)` with slots `sigma_nu, sigma_lambda`
...
The y_rep stochastic refresher (§6a) still needs to know how to sample
...
- If update_nuisance uses RW-MH, tune `prop_sd` so acceptance rate
  lands in [0.2, 0.5] per Linero 2022 §3.4 recommendations.
...
4. **Declare predict DAG edges** into `y_rep` and **register the y_rep
   stochastic refresher** (see §6a above).
...
void set_current(Rcpp::List params) {
    ...
    if (has_X) {
        ...
        N_ = N_new;   // refresh cached N (system_design.md §7 rule 6)
    }
...
```cpp
// LEGACY (system_design §7 rule 4 violation):
...
- `predict_rng_` — used by stochastic refreshers invoked via
  `predict_at` (y_rep sampling, see §6a). Declared `mutable` so that
...
**⚠️ Critical #1 (data_input forwarding):** if the constructor calls
...
**⚠️ Critical #2 (keep_history → history-mode predict):** if the
...
`n_draws × N` matrix per refreshed key. A single-draw `predict_at`
...
Rcpp::List predict_at(Rcpp::List new_data) const {
    ...
    // keys that are in `(data_inputs ∪ block_names)`. Sub-outputs of
...
### Tail template — choose based on §1 Q0 "Runtime backend"
