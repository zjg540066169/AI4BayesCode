...
description: |
  ...
  runs ONLY L1 syntax-retry (up to 5 attempts) -- the L2 17-check
...
1. **Generation** (already done by `sim3/scripts/run_sim3.sh`):
   - 5 independent regens x 20 models = 200 cells
...
2. **Post-hoc validation** (THIS skill):
   - Stage A: per-cell L2 audit (17 checks from `validator.md`)
...
2. Prompt: "Read `<MODEL>.cpp` and `<MODEL>_runner.R`. Apply the
   17 checks from `skills/validator.md` Layer 2. Write a
   single-file report to `l2_report_posthoc.md` with PASS / FAIL /
   N/A per check + brief justification."
3. Pass criterion: 17/17 PASS or N/A. Any FAIL -> cell flagged
   `L2_FAIL_<check_id>`.
...
Implementation: `scripts/run_l2_posthoc.sh` (TODO: write) loops
over `models[1-9]*/*/STATUS == SUCCESS` cells and submits one
short `claude -p` per cell. Token cost: roughly 1/3 of generation
cost (only the audit step, not the generate-then-audit loop).
...
When per-model 5 regens have 5 sim2_compare rows, aggregate as
...
```
Settings 3 results (200 cells = 10 regens x 20 models):
...
1. **Don't skip cells with STATUS=FAILED_L1.** They count as 0/1 in
   the "compile rate" denominator (200 cells x 1 = 200).
...
- `validator.md` -- defines the 17 checks of Stage A and the R1/R2/R3
...
| C -- sim2 compare | local R, ~5-60 min x 20 reps | ~100 hr serial -- heavy; consider only cells passing A+B |
