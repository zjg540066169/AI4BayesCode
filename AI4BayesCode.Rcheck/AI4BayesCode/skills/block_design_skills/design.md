...
Claude Code: AskUserQuestion; elsewhere: the markdown labeled-option fallback from `start.md` §0).
Standing choice labeled **"(default)"**, never "(Recommended)". Always an **"Other"** escape.
Decision-relevant numerical thresholds are **ELICITED** ("(default)"); mechanical test-numerics use recorded, overridable defaults (`validate.md` §5). NOTHING is written
...
`skills/system_design_skills/<module>.md` (resolved via "system_design §N" → the
...
> LOAD: `skills/system_design_skills/geometry.md` (system_design §11). This is the ONE gate the
> validator structurally cannot enforce (§11.3): a geometry violation converges cleanly to the
> WRONG distribution. Get it right here or nowhere.

**Step 1.1 — State the target distribution explicitly** (§11.4 rule 1): prior, likelihood, and
the SUPPORT of every variable. Write it into the block-header comment draft now — every block
header must declare which of §11.1's three supported shapes it samples.

**Step 1.2 — Classify against the boundary** (§11.1 supported / §11.2 NOT supported):
...
| **Dimension-changing / trans-dim** (§11.2a) | support depends on a discrete indicator (spike-and-slab Dirac, model-size, change-point, basis-birth, BNP-K) | **NOT a new ad-hoc block** — STOP |
| **Discrete w/ strong local dependence** (§11.2b) | HMM z_{1:T}, Ising/CRF/MRF, BN structure | **NOT per-site Gibbs** — STOP |

**Step 1.3 — The STOP cases (ask before proceeding).** If §11.2(a) or §11.2(b): a naive
Gibbs-only or NUTS-only block is SILENTLY WRONG (§11.2 spells out both failure modes — Gibbs is
not irreducible across the Dirac; γ-marginalized NUTS hallucinates inclusion probabilities;
...
- §11.2(a) → `rjmcmc_block` (identity-coordinate, Jacobian-free; ref `SpikeSlabRJMCMC.cpp`); or
...
- §11.2(b) → the matching shipped specialized block (`hmm_block`, `ising_cluster_block`,
...
or marginalized NUTS. The variance/scale-prior discipline (§11.6) — Jeffreys default with k=0
half-Normal(0,1) fallback; IG(ε,ε) PROHIBITED as a "noninformative" default — is in force the
moment your block introduces a scale parameter.

> **SIGN-OFF 1 (geometry).** Lock: the stated target, its §11.1/§11.2 class, and (if a STOP
...
> LOAD: `skills/system_design_skills/families.md` (system_design §13) for the family conventions
> and the metric/warmup decision. For a VI block ALSO load `skills/system_design_skills/vi.md`
> (system_design §18). Do not pin to MCMC — `engine_kind_t` admits `MCMC` and `VI`.

**Step 2.1 — Pick the engine kind / family** (structured prompt; default depends on the Stage-1
class). Each family has its own setter naming and Tier-B contract (§13):

| Branch | When | Tier-B contract highlights (§13) |
|---|---|---|
| **NUTS-gradient** (default for fixed-dim continuous) | smooth log-density; hand-written natural-scale logp + grad | `set_current(arma::vec)` unconstrained; gradient Check #12 (semantic AD-verify via a throwaway compile at generation, see `jacobian.md` §10.1) |
| **Gibbs** (closed-form conjugate / discrete / **Gaussian-conditional**) | Exception 1/3 applies (`codegen_priors §2b`); discrete, NUTS-wasteful textbook conjugate, OR a Gaussian / GMRF field where each site has an exact 1-D Gaussian full conditional read off the precision row (mᵢ, vᵢ closed-form) | `params_fn`/`log_odds_fn`/`alpha_post_fn`; Check #15 parity + Check #16/#17 (highest silent-bug risk) |
| **Slice / ESS** | 1-D non-diff/black-box (`univariate_slice_sampling_block`, `codegen_priors §2b.1`), or a GMRF **prior** under a **NON-Gaussian** likelihood (Murray 2010 ESS). NOTE: if the likelihood is Gaussian-conjugate the site conditionals ARE closed-form → use the **Gibbs** row above, not ESS | metric-free; no dense-metric machinery |
| **VI** | OPT-IN (`vi.md` §18) — high-dim with symmetry, or a fast factorized-q summary suffices | derives `vi_block`; `engine_kind ==VI`; writes q-SAMPLE not q-mean to shared_data (§18.4); optimizer = RAABBVI (stochastic ADVI, §18.7) **OR closed-form CAVI** when the mean-field updates are analytic (conjugate-exponential / Gaussian MRF — μᵢ, sᵢ² in closed form, deterministic, no reparam-gradient); PSIS-k̂ not R-hat (§18.8) |
...
> `validator.md §24` / `joint_nuts_failure.md` and `families.md`). There is NO centered+dense escape hatch. If
> your block parameterizes a hierarchical scale × effect, design it non-centered from the start.

There is NO "Check #11.7" — do not cite it (cleared 2026-06-20).

**Non-Gaussian / discrete variational families (e.g. Bernoulli mean-field).** `vi.md` §18 and
...
§18.10 defers a first-class non-Gaussian VI contract to a future version; until then this
slot-mapping is the **documented** path — state it explicitly in the header, it is not a hack
to hide. (PSIS-k̂ can be ill-defined / NaN at a near-point-mass q — report that transparently,
...
> family (mean-field vs full-rank, §18.5) + the q-sample-to-shared_data commitment.
...
topics, DP/BNP clusters — see `label_switching.md` §1), the block MUST sample that
...
- do not silently auto-impose an ordering constraint (`mu_1 < mu_2 < ...`) to force
  ...
  constraint slice in Stage 3, the convention `label_switching.md §2` documents — but the USER
...
see `label_switching.md` (§3–§6). A correct sampler that explores all K! modes is fine; collapsing
them to one labeling is post-processing. If the block faces this: set the manifest flag
**`LabelSwitching: true`** (`skill.md` §3.2) so the analysis / runner layer knows to relabel, AND
record it as a block-local failure-mode note (`skill.md` §2.2) citing `label_switching.md` — never
as in-block logic.
...
> table) and `skills/system_design_skills/jacobian.md` (system_design §10). The universal rule
> (§10): **users NEVER hand-write a Jacobian formula — in NO family.** It is the single most
...
**Step 3.2 — The Jacobian lives in the library, not your code** (`jacobian.md` §10.1): the
...
gradient pair, the **Check #12** semantic AD-verify via a throwaway compile at generation (`jacobian.md` §10.1: paste functions, add
`autodiff::var` twins, compare 5–20 random-θ points < 1e-8, delete on PASS) is mandatory.
...
`σ ~ LogNormal(μ,s)` and you sample `ℓ_σ = log σ`, the CoV is ALREADY baked in — `LogNormal` just
means `log σ ~ Normal(μ,s)`, so the transported prior is `N(ℓ_σ; μ, s²)` with NO extra `+ log σ`.
Adding an explicit `+ log σ` on top of the Normal-on-log density DOUBLE-COUNTS (the posterior mean
of `ℓ_σ` then lands at `μ + s²` instead of `μ` — a clean T4 oracle). Write an explicit CoV term
ONLY when the prior density is given on the natural parameter (HalfNormal / HalfCauchy / Gamma on
`σ` itself), NOT when it is already a log-scale (LogNormal) prior.
...
its contract). The proposal Jacobian is ALSO library-owned (`jacobian.md` §10.2) —
...
**spike-and-slab over a FIXED candidate set of size p**: the "dimension change" is encoded by `γ`
indicators in fixed-`2p` storage (a birth = flip `γ_j` on + draw its slab), NOT genuine
...
proposal terms cancel exactly — the accept ratio reduces to the pure marginal-likelihood (×prior)
...
> LOAD: `skills/system_design_skills/interface.md` (system_design §1–§2) and
> `skills/system_design_skills/lifecycle.md` (system_design §14). **Keep the two contracts
...
C++-only machinery — NONE of these get a `.method ` and R never sees them (interface.md §2,
Tier B). The lifecycle to follow is `lifecycle.md` §14's 7-step (Tier C kernel + license-gate →
...
`readapt_NUTS` ONLY if the composite contains a NUTS-family child (interface.md §1). Tier A's
`set_current(Rcpp::List)` is a pure DISPATCHER routing keys into Tier B's fine-grained C++
setters (interface.md §2; dispatcher contract in `dataflow.md` §7). **There is NEVER a `$set_X`,
`$set_Y`, `$set_sigma`, or any other state-method on the wrapper** (interface.md §1; the §17
...
GPL-3-compatibility gate (`lifecycle.md` §14 Step 1: GPL-3 / GPL-2+ / MIT / BSD / MPL-2.0 /
...
> LOAD: `skills/system_design_skills/dataflow.md` (system_design §3–§9).

- **shared_data sync** (§3): a single-parameter block's `name ` IS its data key. After ANY
 ...
 predict_at the OLD value (§3 invariant).
- **Refreshers** (§5): deterministic (`register_refresher`, pure fn of shared_data, e.g.
 ...
 refresher. R3.b PSIS-LOO is DIAGNOSTIC ONLY — `warning `, never `stopifnot ` (§5).
- **Two DAGs, kept disjoint** (§4): the Gibbs DAG (`declare_dependencies` / `declare_invalidates`
 ...
 NEVER traversed by predict_at and must never overlap `predict_edges_` (§4).
- **RNG separation** (§8): the wrapper carries `rng_` (MCMC, advanced by `step `),
 `mutable predict_rng_` (predict_at only), and — iff NUTS-family — `mutable readapt_rng_`. Seeds
 are the documented SplitMix64-mixed constants (§8). Conflating streams is a silent reproducibility
 bug (Check #13 / Check #23). Note: BART-family uses R's RNG; the mt19937 arg is ignored.
- **History / memory** (§9, §12): `keep_history` set once at construction; leaves never store a
...
`codegen_priors §2b` **Exception 4** (AI-authored custom block/sampler) — NOT the Gibbs
...
The block MUST carry the inline justification comment (verbatim form from `codegen_priors §2b`):
