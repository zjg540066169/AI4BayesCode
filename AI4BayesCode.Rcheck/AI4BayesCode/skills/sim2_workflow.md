...
| Total rows in csv per model | 100 (N_REPLICATES × 1) | 100 (N_REPLICATES × N_SAMPLERS = 20 × 5) |
| Extra csv column | — | `sampler_id ∈ {1..5}` |
...
total, 100 rows per model (20 reps × 5 samplers).
...
- Auto-chdir to the repository root — that's `dirname × 4` from the
  script path:
...
Same as sim1_workflow §3 (FAILED_L3 → fail/), but the routing decision is
