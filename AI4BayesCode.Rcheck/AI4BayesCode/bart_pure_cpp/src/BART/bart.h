...
class bart {
public:
   // Full member initializer lists — DART hyperparameters (dart, const_theta,
   // a, b, rho, theta, omega) MUST be set here so that user code can safely
   // skip setdart() when sparsity is not wanted.  Previously these were
   // uninitialized, which meant reading `dart` or `theta` before setdart()
   // was called triggered undefined behavior.
   bart():m(200),t(m),pi(),p(0),n(0),x(0),y(0),xi(),
          allfit(0),r(0),ftemp(0),di(),
          dart(false),dartOn(false),aug(false),const_theta(false),
          a(0.5),b(1.0),rho(0.),theta(0.),omega(1.),
          accept(0.),grp(0) {};
   bart(size_t im):m(im),t(m),pi(),p(0),n(0),x(0),y(0),xi(),
                   allfit(0),r(0),ftemp(0),di(),
                   dart(false),dartOn(false),aug(false),const_theta(false),
                   a(0.5),b(1.0),rho(0.),theta(0.),omega(1.),
                   accept(0.),grp(0) {};
   bart(const bart& ib):m(ib.m),t(m),pi(ib.pi),p(0),n(0),x(0),y(0),xi(),
                        allfit(0),r(0),ftemp(0),di(),
                        dart(false),dartOn(false),aug(false),const_theta(false),
                        a(0.5),b(1.0),rho(0.),theta(0.),omega(1.),
                        accept(0.),grp(0)
   ...
   const xinfo& getxinfo() const {return xi;}
   ...
   const std::vector<size_t>& getnv() const {return nv;}
   const std::vector<double>& getpv() const {return pv;}
   ...
   // Walks LIVE in-memory trees on currently-stored x to refresh
   // allfit[].  Called after set_tree() (which mutates the live forest
   // externally without touching allfit).  set_X / set_data already
   // refresh allfit via setdata()->predict().  step() incrementally
   // maintains it.  set_Y leaves trees(x) unchanged, no refresh needed.
   void refresh_allfit() {
     if (!allfit || !x) return;
     predict(p, n, x, allfit);
   }
