// SoftBART, Copyright (C) Antonio R. Linero and Yun Yang.
// Linero & Yang (2018), JRSS-B. Licensed GPL-2.0-or-later.
// Vendored into AI4BayesCode.
...
#ifdef NoRcpp
#define RCPP_CHECK_INTERRUPT() ((void)0)
#else
#define RCPP_CHECK_INTERRUPT() Rcpp::checkUserInterrupt()
#endif
...
#endif
  void finalize_from_group();  // pure-C++ derived setup from `group`
  Hypers();
...
class Forest {
 ...
 public:
...
#endif
  Forest(const Hypers& h, const Opts& o);
  ~Forest();
  ...
  // BART_unified additions: const accessors for tree serialization.
  const std::vector<Node*>& get_trees()   const { return trees; }
  const Hypers&             get_hypers()  const { return hypers; }

  // BART_unified addition: replace the current forest with `new_trees`.
  // Caller transfers ownership of the Node* pointers; existing trees are
  // freed.  Tree count must equal current trees.size() (a tighter check
  // happens in softbart_model::set_tree before this is called).
  void set_trees(std::vector<Node*> new_trees) {
