 ...
 *
 *      A) Default is 0.8 (Stan/PyMC/NumPyro mainstream; overrides the
 *         vendored mcmclib default of 0.55).
...
// ---- Subtest A: default target_accept == 0.8 ----------------------------

static void test_A_default_target() {
    std::printf("\n=== A: nuts_block default target_accept_rate = 0.8 ===\n");
    nuts_block_config cfg;
    check(cfg.target_accept_rate == 0.8,
          "A.a nuts_block_config default 0.8",
    ...
    check(blk->current_target_accept() == 0.8,
          "A.b block reports 0.8 after ctor",
          "got " + std::to_string(blk->current_target_accept()));
}

// ---- Subtest B: ctor override to 0.55 --------------------------------------

static void test_B_ctor_override() {
    std::printf("\n=== B: ctor override target_accept_rate = 0.55 ===\n");
    const arma::vec y = gen_data(200, 2.5, 1.2, 20260725ull);
    auto blk = make_mu_block(y, 0.0, /*target_override=*/0.55);
    check(blk->current_target_accept() == 0.55,
          "B.a block reports 0.55 after ctor",
          "got " + std::to_string(blk->current_target_accept()));
    ...
    check(std::isfinite(eps) && eps > 0.0,
          "B.b chain runs and step_size finite under target=0.55",
...
static void test_C_readapt_target_override() {
    ...
    check(blk.current_target_accept() == 0.8,
          "C.a target still 0.8 after 100 steps",
          "got " + std::to_string(blk.current_target_accept()));
    ...
    check(eps_after < eps_before,
          "C.c step_size shrank when target rose 0.8 -> 0.99",
...
// Build a 2-D joint block over (mu, log_sigma) for a Normal model, so we
...
static void test_E_joint_variants() {
    ...
    // E.A: default 0.8
    {
        ...
        check(c.target_accept_rate == 0.8,
              "E.A joint_nuts_block_config default 0.8",
              "got " + std::to_string(c.target_accept_rate));
    }

    const arma::vec y = gen_data(200, 2.5, 1.2, 20260725ull);

    // E.B: ctor override to 0.55 stored and forwarded
    {
        auto blk = std::make_unique<joint_nuts_block>(make_joint_cfg(0.55));
        check(blk->current_target_accept() == 0.55,
              "E.B joint block reports 0.55 after ctor",
              "got " + std::to_string(blk->current_target_accept()));
    }
    ...
    // E.C: readapt override adjusts target and step size direction
    {
        ...
        std::printf("     joint step_size: before=%g  after=%g\n",
                    eps_before, eps_after);
        check(eps_after < eps_before,
              "E.C.b joint step_size shrank when target rose 0.8 -> 0.99",
